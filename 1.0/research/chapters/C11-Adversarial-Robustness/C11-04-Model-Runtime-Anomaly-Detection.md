...
| **11.4.2** | **Verify that** inputs flagged as anomalous trigger gating actions. | 2 | Detection without enforcement -- a flagged-but-still-processed poisoned input still corrupts output or fires a tool action (ATPA post-tool-call manipulation, MCP rug-pulls, SSH-key exfiltration, MCP-ITP implicit redirection into a legitimate high-privilege tool). Auto-approval amplifies impact: ~84.2% tool-poisoning ASR with auto-approve enabled, and CVE-2026-22708 turned Cursor's allowlisted `git branch` into RCE via env-var poisoning. | Confirm flagged inputs are blocked, quarantined, down-ranked, or routed to human review -- not merely logged. Exercise gating layers: NeMo Guardrails input/retrieval/execution rails, Lakera Guard L1--L4, Meta LlamaFirewall, Bedrock contextual grounding, trust-weighted retrieval (ASI06), stateful cross-stage trust scoring (MMA-RAG^T), hard-block on tool-hash mismatch, and the auto-approval kill-switch. Enumerate implicitly trusted operations (shell built-ins, env mutations, config writes) and confirm they are gated too. Inject known-bad content end-to-end and verify the gating action actually fires per route. | Thresholds trade false positives against false negatives (Lakera L1 vs L4); counterfactual gating (RAGuard leave-one-out) costs N+1 inference passes; response-side gating for ATPA is immature; allowlists silently exempt built-ins/env writes (CVE-2026-22708); over-aggressive gating erodes clean-task utility (benchmarks show ~94.3% retention as a practical upper bound when stacking rails). |
