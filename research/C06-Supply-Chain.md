...
> **2025-2026 Highlights:** The AI BOM ecosystem (CycloneDX ML, SPDX AI) is gaining adoption alongside SLSA for ML pipelines. Malicious model discoveries on public registries continued to grow, reinforcing the need for model scanning and SafeTensors format enforcement as baseline controls. As of April 2026, AI agent skill marketplaces have emerged as a major new supply chain attack surface — the OpenClaw/ClawHavoc crisis (1,184 malicious skills, CVE-2026-25253) and AI-powered CI/CD attacks (Hackerbot-claw) demonstrate that supply chain threats are expanding beyond traditional package registries. The LiteLLM/TeamPCP supply chain attack (March 24, 2026) showed how compromising a single security scanner (Trivy) can cascade into backdooring a foundational AI gateway library present in 36% of cloud environments, and the Lazarus/BlueNoroff axios npm compromise (March 30, 2026) reached OpenAI's macOS code-signing pipeline, forcing rotation of ChatGPT Desktop and Codex signing certificates. IBM X-Force 2026 reports supply chain compromises have nearly quadrupled since 2020. Two new requirements landed in this cycle: 6.1.6 formalizes behavioral acceptance testing (UK AISI Inspect, Stanford HELM Safety, DeepEval now make this practical) and 6.4.6 requires cryptographic signing keys to be pinned per registry with explicit re-approval on rotation — directly motivated by the wave of publisher-account-takeover attacks.
...
- **LiteLLM/TeamPCP cascading supply chain attack** — On March 24, 2026, TeamPCP compromised the Trivy security scanner's GitHub Action (v0.69.4) to harvest PyPI credentials from LiteLLM's CI/CD pipeline, then published two backdoored LiteLLM versions (1.82.7, 1.82.8) to PyPI. The three-stage payload harvested SSH keys, cloud credentials, and Kubernetes secrets, deployed privileged pods for lateral movement, and installed a persistent systemd backdoor. LiteLLM averages 3.4 million daily downloads and is present in 36% of cloud environments — the attack also compromised Checkmarx KICS in the same campaign. TeamPCP reportedly collaborates with LAPSUS$.
- **Axios npm maintainer-account hijack (Lazarus/BlueNoroff)** — On March 30, 2026, North Korea's BlueNoroff subgroup (tracked as Sapphire Sleet by Microsoft, attributed HIGH confidence by Elastic Security Labs) social-engineered the lead axios npm maintainer and published backdoored axios versions 1.14.1 and 0.30.4 within a 39-minute window. A single new transitive dependency, `plain-crypto-js@4.2.1`, deployed a cross-platform RAT to macOS/Windows/Linux via a `postinstall` hook that pulled stage-2 payloads from `sfrclak[.]com:8000`. Axios sees roughly 70M weekly downloads; OpenAI's macOS app-signing pipeline pulled the infected code, exposing code-signing certificates for ChatGPT Desktop and Codex and forcing a full signing-certificate rotation. The internal project name `macWebT` links the campaign to BlueNoroff's 2023 RustBucket `webT` module. This incident is the canonical justification for requirement 6.4.6 (publisher-key pinning with explicit rotation re-approval).
...
| Mar 2026 | LiteLLM/TeamPCP supply chain attack — backdoored PyPI packages via Trivy compromise | 3.4M daily downloads; three-stage payload (credential harvester, K8s lateral movement, persistent backdoor); present in 36% of cloud environments | [Hacker News](https://thehackernews.com/2026/03/teampcp-backdoors-litellm-versions.html) |
| Mar 2026 | Axios npm maintainer hijack (BlueNoroff/Sapphire Sleet) — cross-platform RAT via `plain-crypto-js` | 70M weekly downloads; OpenAI macOS code-signing pipeline ingested malicious axios 1.14.1/0.30.4; ChatGPT Desktop + Codex signing certs rotated | [Elastic Security Labs](https://www.elastic.co/security-labs/axios-one-rat-to-rule-them-all) |
| Apr 2026 | Microsoft: Axios compromise attribution + mitigations | Links axios RAT infrastructure to Sapphire Sleet; guidance on `npm audit signatures`, provenance checks, and CI egress controls for `postinstall` hooks | [Microsoft Security Blog](https://www.microsoft.com/en-us/security/blog/2026/04/01/mitigating-the-axios-npm-supply-chain-compromise/) |
...
- [MITRE ATLAS AML.T0010: Supply Chain Compromise](https://atlas.mitre.org/techniques/AML.T0010) — 5 sub-techniques: Hardware (.000), ML Software (.001), Data (.002), Model (.003), Container Registry (.004). ATLAS v5.1.0 (Nov 2025) expanded to 16 tactics / 84 techniques; v5.4.0 (Feb 2026) added "Publish Poisoned AI Agent Tool" and "Escape to Host", plus new case studies including Hugging Face organization-confusion attacks. See [atlas-data releases](https://github.com/mitre-atlas/atlas-data/releases) for the canonical change log.
...
|-----------------|--------------|-------|
| [C01 Training Data](C01-Training-Data.md) | Dataset provenance and poisoning | C6.3 covers supply-chain risk of *external* datasets; C01 covers data quality broadly. ATLAS AML.T0010.002 (Data) links both. EU AI Act Annex IV Section 2 requires documenting data provenance. |
| [C03 Model Lifecycle](C03-Model-Lifecycle-Management.md) | Model versioning, signing, deployment | C6.5 AI BOM complements C03 model registry requirements. OpenSSF Model Signing creates checkpoints at upload, deployment, and downstream reuse. ATLAS AML.T0010.003 covers compromised pretrained models. |
| [C04 Infrastructure](C04-Infrastructure.md) | Workload-level model loading hardening | C4.1.3 format allowlisting and C4.1.4 attestation complement C6.1 vetting and C6.5 AI BOM verification at load time. ATLAS AML.T0010.004 targets container registries. |
...
| [C09 Orchestration & Agents](C09-Orchestration-and-Agents.md) | Agent skill/plugin marketplace supply chain | OpenClaw/ClawHavoc (1,184 malicious skills) demonstrates that AI agent ecosystems create new supply chain attack surfaces beyond traditional package registries. MITRE ATLAS v5.4.0 (Feb 2026) added a dedicated "Publish Poisoned AI Agent Tool" technique to cover this class. C6.2 trusted-source controls — including 6.2.2 publisher-key pinning — apply to skill marketplaces. |
| [C13 Monitoring & Logging](C13-Monitoring-and-Logging.md) | Anomaly detection and incident response | C6.4 monitoring requirements feed into C13 centralized logging. |
...
- [ ] **How do you break cascading CI/CD-to-AI-library attack chains?** — The LiteLLM/TeamPCP attack (March 2026) showed that compromising a single security scanner (Trivy) can cascade into backdooring foundational AI infrastructure with 3.4M daily downloads. Current CI/CD hardening (pinned actions, SLSA attestations) would have mitigated this, but adoption remains uneven. Should AI gateway libraries require higher SLSA levels than typical dependencies?
- [ ] **What does key-pinning-with-rotation (6.4.6) actually look like in practice?** — The Lazarus/BlueNoroff axios hijack (March 30, 2026) compromised a maintainer account and published signed-from-a-legitimate-key backdoors within a 39-minute window, eventually reaching OpenAI's macOS code-signing pipeline. `npm audit signatures`, cosign key policies, and Sigstore trust roots each cover part of the problem, but no registry provides a turnkey "pin publisher keys per source, require human re-approval on rotation" workflow. Until that exists, 6.4.6 depends on organization-internal tooling.
- [ ] **Is behavioral acceptance testing (6.1.6) practical at scale?** — UK AISI Inspect, HELM Safety v1.0, and DeepEval provide frameworks, but running comprehensive safety/alignment/capability boundary tests adds significant CI time. The 2026 International AI Safety Report warns that models can learn to distinguish test environments from deployment — how do we prevent evaluation gaming?
...
- [C06-01 Pretrained Model Vetting](C06-01-Pretrained-Model-Vetting.md) — Section detail for C6.1: signed provenance, malware scanning, quarantine, behavioral acceptance testing, and adversarial fine-tune evaluation.
- [C06-02 Trusted Source Enforcement](C06-02-Trusted-Source-Enforcement.md) — Section detail for C6.2, including 6.2.2 per-registry publisher-key pinning with explicit rotation re-approval.
- [C06-03 Third-Party Dataset Risk](C06-03-Third-Party-Dataset-Risk.md) — Section detail for C6.3: disallowed-content scrubbing, poisoning risk assessment, AI BOM lineage capture, and drift monitoring for external datasets.
- [C03-01 Model Authorization & Integrity](C03-01-Model-Authorization-Integrity.md) — Complementary C03 view: model registry controls, OMS/Sigstore signing at deployment time, and deployment-time signature verification.
- [C01-05 Data Lineage & Traceability](C01-05-Data-Lineage-Traceability.md) — Upstream view of dataset provenance and labeling lineage that feeds the AI BOM fields required by C6.3 and C6.5.
