...
|---|-------------|:-----:|-----------------|----------------------|--------------|
| **2.2.1** | **Verify that** every prompt is scored by a content classifier for violence, self-harm, hate, and sexual content against configurable thresholds. Prompts that exceed those thresholds are rejected or sanitized before reaching the model context. | 1 | Disallowed-content generation and policy bypass via jailbreaks (MITRE ATLAS AML.T0054 LLM Jailbreak; OWASP LLM01:2025). Real bypass families: multi-turn Crescendo (~98% in published runs), Skeleton Key, Bad Likert Judge, narrative reframing (IntentPrompt, EMNLP 2025 Findings), EchoGram token-flip, and "Helpful Mode" role-confusion that coaxes the *guardrail model itself* (Nemotron-Safety-8B, Granite-Guardian-3.2-5B) into emitting the content it should block. Advisory-only screening that flags-but-forwards is itself the vulnerability. | Route every prompt through a classifier (OpenAI `omni-moderation`, Azure AI Content Safety severity levels, Bedrock Guardrails, Llama Guard 4, Granite Guardian) wired as a blocking gateway — confirm a threshold breach yields a hard reject/sanitize *before* the prompt enters model context, not after. Calibrate thresholds with paired benchmarks: MLCommons AILuminate for per-hazard block rate, OR-Bench-Hard and XSTest for over-refusal. Replay archived attack corpora through the full pipeline (not the bare model) each release via Promptfoo, garak, or DeepTeam. SoK: Evaluating Jailbreak Guardrails (arXiv 2506.10597) gives a six-dimension Security-Efficiency-Utility scoring frame. | **High** maturity for classification, but thresholds are an irreducible per-deployment tradeoff between under- and over-blocking. Adversarial paraphrasing still cuts detector TPR ~87.88%; single-message scoring is blind to multi-turn and narrative-reframing trajectories; Trojan-Speak (arXiv 2603.29038) shows adversarial fine-tuning defeats LLM-as-classifier defenses outright when the attacker controls the weights. Pair classifiers with deterministic policy gates and capability limits — never trust a single LLM judge. Screening decisions and rejections should be logged to the monitoring/SIEM stack (logging requirements live in C12). |
| **2.2.2** | **Verify that** prompt content classification is evaluated for unsupported languages. | 1 | Low-resource, code-switched, and transliterated prompts slip past English-centric classifiers. ACL 2025 code-switching red-teaming produced 46.7% more successful attacks than English; MNMR-GenA and morphology-aware generators target low-resource locales specifically; provider docs explicitly mark unsupported languages as *ineffective* (e.g., Bedrock's supported-language matrix), so an unscreened locale is a silent open door rather than a degraded one. | Cross-map the deployment's language footprint against each guardrail's documented coverage (Azure's 8 specially-trained languages; Bedrock standard vs. classic tiers; Model Armor's 9 tested languages). Run a language detector (fastText 176-language, CLD3, or Azure Language Detection) ahead of moderation and route unknown / low-confidence / unsupported locales to block, escalate, or English-proxy safety reasoning. Measure per-locale false-negative rate with PolyGuardPrompts (17 languages, 29K samples) and MrGuard. Confirm logs capture the detected language code plus confidence for audit. | **Medium**. Detectors, support matrices, and benchmark corpora exist, but teams rarely set per-locale thresholds. Code-switching, transliteration, short-message ambiguity, and text-in-image attacks (which double as a language bypass for low-resource locales) remain practical. PolyGuard (COLM 2025) and Llama Prompt Guard 2 86M (8 evaluated languages) widen open-weight coverage; CLD3's upstream repo is archived, so don't make it the sole control. |
| **2.2.3** | **Verify that** non-text inputs (image/video/audio) are checked for adversarial perturbations, steganographic payloads, hidden or embedded content, or known attack patterns. | 2 | Text-in-image typographic injection (FigStep, AAAI 2025), image-scaling attacks where the payload only emerges after downscaling (Trail of Bits Anamorpher, Aug 2025 — drove Gemini CLI to exfiltrate calendar data), "unseeable" low-contrast screenshot injection (Brave, Oct 2025), steganographic payloads (Odysseus dual-steganography against commercial multimodal systems, arXiv 2512.20168), near-ultrasonic and adversarial audio (Sirens' Whisper, AudioJailbreak), and video-driven prompting (CVPR 2026). The preprocessing pipeline itself becomes the attack surface. | OCR-then-classify every inbound image through the same text guardrails, wrapped in delimiters and marked untrusted. Decode-and-re-encode to normalize file structure and strip metadata channels (ExifTool `-all=`, libvips/ImageMagick `-strip`, Pillow re-save), but do not treat transformation as proof that pixel-domain steganography is absent. Prefer rejecting oversized images or limiting dimensions before any downscale; where downscaling is unavoidable, preview and screen the exact image the model receives, and require explicit user confirmation before image-derived content can trigger sensitive actions. Add CDR (OPSWAT MetaDefender Deep CDR) for file-borne hidden content. Use multimodal classifiers where available (Llama Guard 4, ShieldGemma 2, Azure `imageWithText`, Bedrock multimodal toxicity GA, and [Google Cloud Model Armor image screening](https://docs.cloud.google.com/model-armor/sanitize-prompts-responses)). For Model Armor's preview, test both visible harmful imagery and OCR-readable text in JPEG, PNG, and BMP inputs, then verify the enforcement point fails closed for files over 4 MB, unsupported formats, embedded images, multiple images, and unavailable regions instead of treating an unscanned input as safe. Transcribe-then-classify audio (Whisper, Azure Speech). Red-team with garak `visual_jailbreak` (FigStep), PyRIT image/audio converters, and Promptfoo image/audio/video strategies. | **Medium** for classification, **Low** for steganalysis. No cloud vendor ships real-time steganalysis; open tools (Aletheia, StegExpose, zsteg) are research-grade (~50% accuracy vs. modern embeddings); ART input transforms are defeatable by adaptive attacks. Model Armor's June 2026 image preview adds visual scanning and OCR, but only for one qualifying image in the `us` and `eu` multi-regions and does not screen images embedded in files or jointly process image-plus-text through its standard prompt/response methods. Transcribe-then-classify is blind to the acoustic channel where near-ultrasonic payloads live — audio-native research guardrails (AudioGuard, arXiv 2604.08867, which reads unsafe sound events and speaker identity straight from the waveform) and explainable image guardrails (SafeVision) are starting to close that gap but remain research-grade. Normalization, metadata stripping, and CDR can reduce known channels but do not prove hidden content is absent; treat both transformation and detection as best-effort. For scaling attacks, avoid downscaling where possible, preview the exact transformed input, and prevent untrusted image content from directly authorizing sensitive actions. |
| **2.2.4** | **Verify that** coordinated attacks spanning multiple input types (e.g., steganographic payloads in images combined with prompt injection in text) are detected and blocked. | 3 | Split-payload attacks that stay benign in any single modality. Canonical case: EchoLeak (CVE-2025-32711), a zero-click M365 Copilot exfiltration where a text payload slipped past the XPIA classifier and an auto-fetched image carried the data out. Also image-scaling chained into tool calls, Odysseus dual-steganography, JPRO multi-agent multimodal jailbreaking (arXiv 2511.07315), and cross-modal implicit-malicious composition (CrossGuard). | Score modalities jointly per request where the platform allows it (`omni-moderation` text+image, Azure `imageWithText` preview, Bedrock combined calls, Llama Guard 4 multi-image). For Level 3, assemble OCR + transcription + joint classification + session-level correlation, and evaluate emerging omni-modal reasoning guardrails (OmniGuard, arXiv 2512.02306, 210K cross-modal samples across text/image/video/audio; UniGuard; GuardReasoner-Omni). Generate coordinated split payloads with PyRIT converters, garak, and Promptfoo, and confirm violating inputs are **blocked before tool/MCP propagation**, not merely flagged. | **Emerging** / Level 3. Per-request joint scoring exists, but nothing productized correlates payload fragments across turns, sessions, or agent hops — the EchoLeak failure mode. Correlation engines (CrossGuard, GuardReasoner-Omni, OmniGuard, UniGuard) remain research/early. Expect to build this control yourself from OCR, transcription, joint classification, and session analytics. |
...
| **Google Cloud Model Armor** | Google | Hate, harassment, sexually explicit, dangerous, CSAM, prompt injection/jailbreak (now up to 10,000 tokens, with improved high-confidence detection), sensitive data, malicious URLs, and image screening | Tested for responsible-safety and prompt-injection filters in Chinese, English, French, German, Italian, Japanese, Korean, Portuguese, and Spanish; quality may vary elsewhere | Screens prompts, responses, documents, and, as of June 25, 2026, images in preview; image screening combines visual scanning with OCR in the `us` and `eu` multi-regions. Supports separate input/output templates, inspect-only vs inspect-and-block enforcement, Cloud Logging, Terraform-managed floor settings via Security Command Center, and regional data residency. Streaming text sanitization became GA on July 10, 2026 ([release notes](https://docs.cloud.google.com/model-armor/release-notes)). | Standalone / Security Command Center |
...
| Tool / Pattern | Coverage | Use in 2.2.2 Verification |
|----------------|----------|-----------------------------|
...
### Multimodal & Hidden-Content Screening (2.2.3 / 2.2.4)
...
| **Google Cloud Model Armor image screening** | Commercial (preview) | Added June 25, 2026; combines visual scanning through advanced Sensitive Data Protection with OCR. It accepts one JPEG, PNG, or BMP image up to 4 MB in the `us` and `eu` multi-regions, but does not screen images embedded in files or jointly screen image-plus-text through `SanitizeUserPrompt` / `SanitizeModelResponse`; exercise each unsupported path and require an explicit failure or compensating scanner ([documentation](https://docs.cloud.google.com/model-armor/overview)). |
| **Image re-encode / resize / strip** | Pattern (ExifTool `-all=`, libvips/ImageMagick `-strip`, Pillow re-save) | Decode and re-encode every inbound image to normalize the file and remove EXIF/XMP/ICC metadata channels cheaply. This can disrupt some known steganographic encodings but is not proof that pixel-domain payloads are absent. Do not rely on rerunning the production downscaler to neutralize scaling attacks; avoid downscaling or preview and screen the exact model-visible result. |
...
| **SafeVision** | Research (arXiv 2510.23960) | Image guardrail that adds semantic reasoning over pure feature classification: a Classification Mode for fast screening and a Comprehension Mode that returns human-readable explanations, with policy-following adaptation to new harm categories via the prompt (no retraining). Reports +8.6% on VisionHarm-T and +15.5% on VisionHarm-C over GPT-4o at >16x the speed; useful when image-screening decisions must be explainable for audit rather than a bare allow/block. |
| **Transcribe-then-classify** | Architectural pattern | STT (Whisper, Azure Speech, Amazon Transcribe) feeding text guardrails — the de facto audio screening pattern since most mainstream moderation APIs classify only transcripts; blind to acoustic-channel payloads, so pair with adversarial-audio testing and audio-native classification. |
| **AudioGuard** | Research (arXiv 2604.08867, April 2026) | Modular audio guardrail that finally addresses the acoustic channel transcribe-then-classify throws away: SoundGuard reads audio-native cues (unsafe sound events, speaker identity) straight from the waveform, ContentGuard handles ASR transcript analysis, and a compositional layer fuses them. Covers non-speech harmful sounds (gunfire, distress, sexual audio), speaker attributes (child voice, celebrity impersonation), and voice+content compositional risk. Reports 0.871 average accuracy across five benchmarks vs. 0.740 for Gemini 3, at ~1.42s vs. ~3.25s for baselines — early but the most direct answer yet to near-ultrasonic/adversarial-audio gaps in 2.2.3. |
...
Threshold tuning (2.2.1) needs paired measurement of under-blocking and over-blocking; these public benchmarks make that measurable rather than vibes-based.
...
| **PolyGuardPrompts** (COLM 2025) | 29K samples across 17 languages | Per-locale false-negative measurement for 2.2.2 unsupported-language gap analysis |
...
| **May 2026** | Microsoft published "Prompts Become Shells," detailing remote-code-execution vulnerabilities in AI agent frameworks where unscreened user input flowed into tool execution paths. | Concrete evidence for why violating inputs must be blocked before tool/MCP propagation, not merely flagged (2.2.1, 2.2.4). |
...
| **March 2026** | Australia's eSafety Commissioner reported AI companion apps exposing children to sexualised content and inadequate crisis routing, feeding directly into enforceable industry codes. | Age-context thresholds (2.2.1) and human-review routing are now regulatory expectations, not just best practice. |
...
|-------------|----------|-------|
| **2.2.1** Content classifiers, thresholds & pre-model rejection | **High (classification) / Medium (rejection-path hardening)** | Multiple production-ready commercial APIs (OpenAI free, Azure, AWS with multimodal toxicity and cross-account safeguards, Google Cloud Model Armor with 10k-token prompt-injection windows and Terraform floor settings, Galileo, Lakera) and open-source options (LLM Guard, Granite Guardian, Llama Guard 4, Llama Prompt Guard 2, ExpGuard, PolyGuard); Anthropic's Constitutional Classifiers++ (January 2026) demonstrates ~40x compute reduction with a 0.05% production refusal rate, and Mozilla's `any-guardrail` gives a uniform interface for A/B-testing detectors. The classifier-as-gateway pattern (block, not advise) is well understood, but multi-turn attacks (Crescendo, Bad Likert Judge), autonomous reasoning-model jailbreak agents, optimized-suffix and token-flip attacks (EchoGram), special-token manipulation (MetaBreak), narrative reframing (IntentPrompt), "Helpful Mode" guardrail role-confusion, and adversarial fine-tuning (Trojan-Speak) all expose weaknesses. Rejection paths must therefore combine classifier screening with deterministic policy gates, capability restrictions, and human approval for high-impact actions. Screening decisions should flow to the monitoring/SIEM stack — SOC-grade tracing is now available off-the-shelf (Datadog LLM Observability, LangWatch, Azure Content Safety Studio, WhyLabs), with logging requirements covered in **C12**. |
| **2.2.2** Unsupported-language abuse testing | **Medium** | Language detection, provider support matrices, and red-team corpora are available, but production teams often lack per-locale thresholds and low-resource language coverage. PolyGuard (COLM 2025, 17 languages with a 29K-sample benchmark) and Meta's Llama Prompt Guard 2 86M (eight evaluated languages) materially expand open-weight multilingual options, but code-switching, transliteration, short-message ambiguity, text-in-image attacks, and morphology-aware low-resource jailbreak generation remain practical bypass paths. |
| **2.2.3** Non-text input screening | **Medium for classification, Low for steganalysis** | Multimodal safety classification is genuinely available: Llama Guard 4 (April 2025), ShieldGemma 2, omni-moderation image support, Azure Image Analyze (GA), Bedrock multimodal toxicity (GA March 2025), and Google Cloud Model Armor image screening (preview June 2026). Hidden-content and adversarial-perturbation detection is much weaker: no cloud vendor ships real-time steganalysis, open steganalysis tools are research-grade, and ART-style input transformations are defeatable by adaptive attacks. Model Armor's visual-scan-plus-OCR path narrows the hyperscaler gap, but its one-image, format, size, region, embedded-image, and image-plus-text limits make explicit fail-closed testing necessary. Normalization, metadata stripping, CDR, and OCR-then-classify are deployable risk-reduction measures, not proof that hidden image content is absent; image-scaling defenses should avoid downscaling where possible or preview and screen the exact model-visible result. Audio still reduces to transcribe-then-classify in production, though waveform-native research guardrails (AudioGuard, April 2026) and explainable image guardrails (SafeVision) point at the acoustic and semantic-reasoning gaps respectively. |
| **2.2.4** Cross-modal coordinated attack detection | **Emerging** | Joint text+image scoring exists per-request (omni-moderation, Azure `imageWithText` preview, Bedrock combined calls, Llama Guard 4 multi-image), and red-team tooling can generate split-payload attacks (garak FigStep probes, PyRIT converters, Promptfoo strategies). But cross-modal correlation engines remain research/early — CrossGuard, GuardReasoner-Omni, and the omni-modal reasoning guardrails OmniGuard (arXiv 2512.02306, 210K cross-modal samples) and UniGuard show productizable progress, yet nothing shipping correlates payload fragments across turns, sessions, or agent hops. Expect to assemble this Level 3 control from OCR, transcription, joint classification, and session analytics. |
...
| [C09 Orchestration & Agentic Action](../C09-Orchestration-and-Agents/C09-Orchestration-and-Agents.md) | Rejected content must not propagate through agent chains before tool execution (2.2.1, 2.2.4) |
| [C10 MCP Security](../C10-MCP-Security/C10-MCP-Security.md) | Content screening must cover MCP tool inputs/outputs; policy-violating content must not propagate to MCP servers (2.2.1) |
| [C12 Monitoring & Logging](../C12-Monitoring-and-Logging/C12-Monitoring-and-Logging.md) | Screening decisions and rejections feed the broader monitoring and SIEM infrastructure; logging requirements live in C12 |
| [C09-02 High-Impact Action Approval](../C09-Orchestration-and-Agents/C09-02-High-Impact-Action-Approval.md) | Borderline content-screening decisions may require human-in-the-loop escalation before a high-impact action proceeds |

---

## Related Pages

- [C11-01 Model Alignment & Safety](../C11-Adversarial-Robustness/C11-01-Model-Alignment-Safety.md) — Extends classifier hardening with alignment, jailbreak resistance, and evidence that safety behavior survives targeted adversarial probing.
- [C07-03 Output Safety, Privacy & Explainability](../C07-Model-Behavior/C07-03-Output-Safety-Privacy-Explainability.md) — Complements input-side screening with post-response checks for unsafe content, PII, and covert exfiltration after generation.
...
- [C12-02 Abuse Detection & Alerting](../C12-Monitoring-and-Logging/C12-02-Abuse-Detection-Alerting.md) — Consumes screening logs and rejected-prompt signals for replay, regression testing, and SOC alerting on emerging bypass patterns.
- [C09-03 Tool and Plugin Isolation](../C09-Orchestration-and-Agents/C09-03-Tool-and-Plugin-Isolation.md) — Keeps screened-out content from reaching tool, MCP, and sub-agent execution paths before downstream trust is granted.
...
| **TAKE IT DOWN Act (U.S. federal)** | Notice-and-removal duty for covered platforms; criminal prohibition on non-consensual intimate imagery | Covered platforms must operate a notice-and-removal process and take down reported non-consensual intimate imagery — including AI-generated "digital forgeries" / deepfakes — within 48 hours of a valid request, with the platform compliance window opening May 19, 2026; criminalizes knowing publication of NCII of minors or non-consenting adults. Pushes deepfake/sexual-content screening (2.2.1, 2.2.3) from best practice into a takedown-SLA obligation on generation and hosting surfaces | FTC enforcement of the platform notice-and-removal duty; federal criminal liability for publication |
...
- [Google Cloud Model Armor — Sanitize Prompts Containing Images](https://docs.cloud.google.com/model-armor/sanitize-prompts-responses)
...
- [Guardrail "Helpful Mode" Role-Confusion Jailbreak — Promptfoo LLM Security Database](https://www.promptfoo.dev/lm-security-db/vuln/guardrail-helpful-mode-jailbreak-10383949)
- [Jailbreaking LLMs Against Moderation Guardrails via Cipher Characters (JAM, OpenReview)](https://openreview.net/forum?id=AcBLtTKK5q)
...
- [OmniGuard: Unified Omni-Modal Guardrails with Deliberate Reasoning (arXiv 2512.02306)](https://arxiv.org/abs/2512.02306)
- [UniGuard: Towards Universal Safety Guardrails for Jailbreak Attacks on Multimodal LLMs (arXiv 2411.01703)](https://arxiv.org/abs/2411.01703)
- [Odysseus: Jailbreaking Commercial Multimodal LLM-Integrated Systems via Dual Steganography (arXiv 2512.20168)](https://arxiv.org/abs/2512.20168)
- [Invisible Safety Threat: Malicious Finetuning for LLM via Steganography (ICLR 2026 Oral, arXiv 2603.08104)](https://arxiv.org/abs/2603.08104)
...
- [AudioGuard: Toward Comprehensive Audio Safety Protection Across Diverse Threat Models (arXiv 2604.08867, April 2026)](https://arxiv.org/abs/2604.08867)
- [SafeVision: Efficient Image Guardrail with Robust Policy Adherence and Explainability (arXiv 2510.23960)](https://arxiv.org/abs/2510.23960)
- [Greenberg Traurig — European Commission Details Article 50 Transparency Obligations (June 2026)](https://www.gtlaw.com/en/insights/2026/6/deepfakes-chatbots-ai-generated-text-european-commission-details-transparency-obligations-under-the-ai-act)
...
- [Crowell & Moring — Federal and State Regulators Target AI Chatbots and Intimate Imagery (TAKE IT DOWN Act, 2026)](https://www.crowell.com/en/insights/client-alerts/federal-and-state-regulators-target-ai-chatbots-and-intimate-imagery)
...
- The guardrail model is itself becoming an attack surface: "Helpful Mode" role-confusion can coax safety classifiers (Nemotron-Safety-8B, Granite-Guardian-3.2-5B) into generating the very content they should block, and steganographic malicious fine-tuning (ICLR 2026) lets a model emit hidden harmful payloads while appearing aligned. When the classifier and the model share a substrate, how should screening architectures preserve a trustworthy, independent judgment boundary — separate weights, deterministic gates, or ensemble disagreement detection?
- Normalize-to-text (OCR plus transcription) remains the practical baseline, but it discards the pixel and acoustic channels where adversarial payloads actually live — image-scaling attacks and near-ultrasonic injection both exploit exactly what normalization throws away. Google Model Armor's June 2026 visual-scan-plus-OCR preview starts to combine those paths for a single image, but not embedded images, multiple images, image-plus-text prompt correlation, audio, or cross-session evidence. Can joint multimodal classifiers close those gaps, or does coordinated-attack detection (2.2.4) inherently require session-level correlation infrastructure that no vendor ships yet?
