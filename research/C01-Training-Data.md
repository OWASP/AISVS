...
> **2025-2026 Highlights:** The training data threat landscape sharpened dramatically. Anthropic/UK AISI/Turing (Oct 2025) showed only 250 documents can backdoor any-size LLM — absolute count matters, not proportion. Nature Medicine (Dec 2024) showed 0.001% medical token poisoning evades standard benchmarks entirely. The Virus Infection Attack (NeurIPS 2025) proved poisoning propagates through synthetic data pipelines, amplifying across model generations. On the regulatory front, California AB 2013 (effective Jan 2026) mandates training data documentation, EDPB made right to erasure its 2025 coordinated enforcement priority, and the FTC's algorithmic disgorgement pattern (6 enforcement actions) makes lineage a regulatory survival control. Tooling advanced with Datasig (Trail of Bits) for dataset fingerprinting, CodeScan (Mar 2026) as the first poisoning scanner for code gen models, lakeFS acquiring DVC (Nov 2025), and OpenLineage gaining traction as the lineage standard.
