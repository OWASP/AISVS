...
- [ ] GPUHammer demonstrated that GDDR6 GPU memory is vulnerable to Rowhammer bit-flip attacks that can silently corrupt model weights. Should cloud providers mandate ECC-capable GPUs (HBM3, GDDR7) for multi-tenant AI inference? Early research (Spoczynski & Melara 2025) proposes GPU-resident cryptographic verification using Tensor Cores, but no vendor-neutral standard exists yet -- what verification frequency and overhead are acceptable for production inference workloads?
