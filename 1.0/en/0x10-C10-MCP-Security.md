...
| **10.3.6** | **Verify that** MCP messages carry a cryptographic signature over the canonicalized payload to enable end-to-end integrity verification independent of transport security. This requirement applies specifically to deployments where TLS is terminated by intermediaries, proxies, or gateways. | 3 | D/V |
...
| **10.4.9** | **Verify that** MCP servers enforce application-layer replay protection by requiring a unique nonce and timestamp per message and rejecting messages that are duplicates or outside an acceptable time window. | 2 | D/V |
...
| **10.6.4** | **Verify that** MCP servers default to fail-closed behavior, rejecting requests when security checks cannot be completed, including signature verification failures, authorization service unavailability, and token validation errors. | 2 | D/V |
