...
This chapter focuses on controls unique to AI model lifecycle management: model registries with AIBOM/MBOM generation, model artifact signing and admission verification, validation and safety evaluation gates, controlled rollout with atomic rollback of model state (including AI-specific runtime state such as KV caches, prompt caches, and retrieval indices), hosted-model identity tracking when weights are not under the organization's control, and authorization and integrity controls for fine-tuning pipelines and RLHF reward models. Generic software change-management controls (code signing infrastructure and key custody, source control review and branch protection, container and artifact registry hardening, build pipeline security, configuration management, environment isolation, and CI/CD audit logging) are covered by ASVS v5 (V13, V14, V15, V16), SLSA, and OWASP SCVS, and are not repeated here. Training-data lineage and dataset integrity are covered by C1; vetting and origin verification of third-party pretrained models are covered by C6; AI-specific incident response and audit logging are covered by C13; and the kill-switch and intermediate-state mechanisms invoked by C3.3.4 and C3.3.5 are defined in C14.1.
