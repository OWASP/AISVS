...
Generic software supply chain controls (dependency scanning, version pinning, lockfile enforcement, container digest pinning, build attestation, reproducible builds, SBOM generation, CI/CD audit logging, etc.) are covered by ASVS v5 (V13, V15), OWASP SCVS, SLSA, and CIS Controls, and are not repeated here. This chapter focuses on supply chain risks unique to AI: model artifact integrity, backdoor detection in pretrained weights, dataset poisoning, AI-specific bills of materials, and model-publisher trust. First-party training-data lineage and dataset integrity are covered by C1; once a third-party artifact has been admitted, the model registry, signing of derivative artifacts, deployment admission, and rollback are covered by C3; and AI-specific incident detection that consumes the threat-intelligence signals defined here is covered by C13.
