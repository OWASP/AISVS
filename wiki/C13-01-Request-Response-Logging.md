...
The OpenTelemetry GenAI Semantic Conventions (stable as of OTel spec v1.40.0, 2025) define a vendor-neutral schema for AI interaction telemetry. Teams should adopt these conventions to ensure interoperability across observability backends. Key attributes include:
...
As of March 2026, the GenAI semantic conventions have reached **v1.40.0** with several additions beyond the original stable core: retrieval spans for RAG pipelines, evaluation events for capturing scoring results inline with traces, reasoning content message parts for chain-of-thought visibility, and `gen_ai.embeddings.dimension.count` for embedding operations. Agent framework conventions (`create_agent`, `invoke_agent`) remain in Development status but are broadly adopted by instrumentation libraries.
...
- **OpenLLMetry** (by Traceloop, 6,600+ GitHub stars) -- Open-source instrumentation library built on OpenTelemetry that auto-instruments LLM frameworks (LangChain, LlamaIndex, OpenAI SDK) and emits GenAI semantic convention-compliant spans. Supported by Dynatrace, Grafana, SigNoz, and other OTel-native backends.
...
- **Langfuse** -- Open-source (MIT) LLM observability with tracing, prompt management, and evaluation scoring. Now part of the ClickHouse ecosystem, leveraging ClickHouse's columnar storage for high-throughput trace ingestion. Self-hostable with SDKs for Python, TypeScript, Go, and Ruby.
...
On March 24, 2026, the widely-used LiteLLM Python package (an LLM API proxy gateway with ~3.4 million daily downloads) was compromised via a supply chain attack. The threat actor TeamPCP hijacked maintainer accounts through a compromised Trivy GitHub Action in LiteLLM's CI/CD pipeline and published backdoored versions (1.82.7, 1.82.8) to PyPI.

The malicious payload was embedded in `litellm/proxy/proxy_server.py` -- the exact component that sits between applications and LLM providers, routing and logging all inference traffic. The backdoor harvested SSH keys, cloud API tokens, Kubernetes secrets, crypto wallets, and `.env` files, then exfiltrated them encrypted with a hardcoded RSA key to a lookalike domain (`models.litellm.cloud`).

This incident has direct implications for requirement 13.1.2 (secure log storage) and 13.1.3 (encryption in transit):
...
- **Dependency integrity verification** (pinning, hash checking, SBOM monitoring) is essential for any component in the logging data path. The malicious versions were live for approximately three hours before PyPI quarantine.
- **Version 1.82.8 used a `.pth` file injection** that executes whenever Python starts, meaning the backdoor persisted beyond just importing litellm -- a technique that evades typical import-level monitoring.

Teams using LLM proxy gateways for centralized logging should treat them as security-critical infrastructure subject to the same supply chain controls as other privileged components (see C06 Supply Chain).
