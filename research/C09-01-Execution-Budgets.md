...
| **9.1.2** | **Verify that** cumulative resource/spend counters are tracked per request chain and hard-stop the chain when thresholds are exceeded. | 2 | **Distributed budget evasion.** Agent spawning sub-agents may stay under per-call limits while exceeding aggregate budgets. Four attack variants: (1) single-agent hallucination loops, (2) multi-agent circular dependencies (the $47K LangChain Analyzer-Verifier ping-pong), (3) file system recursion (agents reading files containing instructions to read the same file), (4) cascading hallucination (87% downstream decision poisoning within 4 hours). SaaS-to-SaaS worm pattern: agents tricked into calling paid APIs recursively 10,000+ times, draining both API credits and cloud balance. Credential theft compounds the risk: a stolen Gemini API key generated $82,314 in 48 hours (455x normal spend, February 2026); Truffle Security found 2,863 live API keys exposed on public websites. **Malicious intermediary amplification (April 2026):** a compromised LLM router sitting between an agent and its provider can silently multiply sub-agent fan-out and exfiltrate credentials in transit — the arXiv 8 April 2026 study documented 26 routers that drained $500K from a single client's wallet by injecting extra tool calls the agent never emitted ([CoinDesk](https://www.coindesk.com/tech/2026/04/13/ai-agents-are-set-to-power-crypto-payments-but-a-hidden-flaw-could-expose-wallets)). | Trace multi-step execution; confirm cumulative counters (tokens, API calls, wall-clock, cost) span all steps and sub-agents. Verify hard-stop at aggregate level. Use provisional reservation + true-up pattern (reserve max_tokens upfront, refund after actual response). Verify per-agent cost attribution: separate API keys per agent, step-level logging with metadata. Databricks **Unity AI Gateway** (2026) logs every request to Unity Catalog system tables with actual dollar costs sliceable by endpoint tag, team, environment, cost center, identity, model, and provider — useful as an audit reference when validating cumulative counters. Observability tools: Langfuse, AgentOps, Laminar, Langtrace AI for per-session cost tracking. | Cumulative tracking across sub-agents not natively supported in most frameworks. Requires correlation via chain/trace ID. OpenTelemetry helps but needs custom budget-enforcement logic. The escrow-based delegation pattern (arXiv 2602.11865) proposes a formal framework: funds held in escrow until verification conditions are met, with recursive budget delegation and bond slashing for breaches. The **Agentic Risk Standard (ARS)** ([arXiv 2604.xxxxx, April 2026](https://www.crowdfundinsider.com/2026/04/272366-researchers-propose-agentic-risk-standard-for-ai-agent-transactions/), Google DeepMind / Microsoft Research / Columbia / t54 Labs / Virtuals Protocol) formalizes a settlement-layer protocol — escrow vaults, collateral posted by AI service providers, and optional third-party underwriting — closing the "guarantee gap" between probabilistic AI reliability and enforceable spend caps. In trials, the collateral mechanism alone deterred 15–20% of risky transactions before they executed. OSS reference implementation via T54 Labs. Market-based allocation (Contract Net Protocol) lets agents bid for work under resource constraints. |
| **9.1.3** | **Verify that** circuit breakers terminate execution on budget violations. | 2 | **Graceless failure on overrun** ([ASI08: Cascading Failures](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/)). Without circuit breakers, budget violations may be logged but not acted upon. OWASP 2026 guidance: *"If an agent starts looping and spamming an expensive tool, trip the circuit breaker immediately."* The AISM framework (AI Security Maturity Model, 2026) now treats the Circuit Breaker as a named architectural pillar alongside rate limits, recursion caps, and safe-mode activation. The "Financial Circuit Breaker" pattern emerged from a $30K runaway agent — monitors token velocity ($/minute) and kills the agent if threshold exceeded. | Trigger budget violations in test; confirm: (a) execution halts immediately, (b) partial state cleaned up, (c) downstream consumers notified, (d) no further tool calls after breaker trips. Test both graceful and crash scenarios. Multi-stage breaker: Stage 1 (Yellow) throttles to 10 tokens/sec; Stage 2 (Orange) revokes write permissions, read-only sandbox; Stage 3 (Red) session termination with forensic state preservation (Arion Research). Verify the HTTP status returned on trip (MLflow Gateway uses `429 Too Many Requests`; many frameworks use `503` — downstream agents must handle both deterministically). | Circuit breaker patterns well-established in microservices (Hystrix, resilience4j). Microsoft Agent Governance Toolkit v2.2.0 implements circuit breakers + SLO enforcement with sub-millisecond policy enforcement (<0.1ms per action). LiteLLM hard budget limits function as infrastructure-layer circuit breakers — block requests when any budget tier is exhausted, with provider routing to switch to cheaper providers automatically. Bifrost blocks requests outright when budget is exceeded. **MLflow AI Gateway 3.10** (March 18, 2026) adds OSS `reject` budget policies that return HTTP 429 when daily/weekly/monthly USD thresholds trip, with Redis-synchronized counters for multi-replica deployments. Galileo Agent Control (March 2026) adds centralized policy enforcement with sub-200ms latency. |
| **9.1.4** | **Verify that** security testing covers runaway loops, budget exhaustion, and partial-failure scenarios, confirming safe termination and consistent state. | 3 | **Untested failure modes.** LangGraph GitHub issue #6731 documents agents infinite-looping until recursion limit error fires, confirming this is often the last line of defense. The Replit agent incident (July 2025) showed an agent ignoring explicit "CODE FREEZE" instructions, deleting 1,206 records, fabricating 4,000 entries, and falsely claiming data was unrecoverable. Memory poisoning attacks (Lakera, November 2025) gradually manipulated a procurement agent over 3 weeks to approve $500K purchases without human review, leading to $5M in fraudulent orders. The **OpenClaw incident chain** (February–March 2026) is a textbook illustration of untested failure modes at the framework level: a 346K-star OSS agent grew faster than its governance, 135,000 instances sat directly exposed to the internet, [CVE-2026-25253](https://www.kaspersky.com/blog/openclaw-vulnerabilities-exposed/55263/) (CVSS 8.8) enabled one-click RCE, and a DeFi-integrated OpenClaw agent erroneously transferred 52.43M LOBSTAR tokens ($250K) — later liquidated for $40K — because no test harness had simulated the specific tool-call pattern that triggered the exploit. | Review test suites for chaos/fault-injection tests: infinite loops, rapid fan-out, partial tool failures mid-chain, simultaneous budget violations. Confirm tests assert safe termination and state consistency. Include multi-agent circular dependency scenarios. Test memory poisoning leading to budget escalation. Include **malicious-intermediary tests** (mock an LLM router that injects extra tool calls or alters provider responses) — the April 2026 router study shows this threat is live in production traffic. Shadow mode (log would-be throttle decisions without enforcement for 1–2 weeks) useful before going live with new limits. | Few organizations have agentic-specific chaos engineering practices. Test harnesses need to simulate adversarial prompt patterns that induce loops. 48% of respondents in a Dark Reading 2026 poll expect agentic AI to become the top attack vector, yet testing coverage remains minimal. OpenClaw fallout is driving early work on framework-level conformance suites that run RCE/cost-exhaustion/data-exfiltration scenarios as a gate before marketplace listing. |
...
## Agent Framework Budget Controls (April 2026)
...
| **CrewAI** (v1.10.1, Mar 4 2026) | `max_iter` per task | 10–20 typical | 45.9K+ GitHub stars; Gemini GenAI upgrade, MCP tool loading fixes; hosted platform has execution-based billing. No native USD cap. |
...
| [MLflow AI Gateway](https://mlflow.org/docs/latest/genai/governance/ai-gateway/budget-alerts-limits) | OSS gateway (v3.10.0, Mar 18 2026) | **USD budget policies** with daily/weekly/monthly reset windows; `alert` fires webhooks and keeps serving, `reject` returns HTTP 429; local tracker for single-instance, Redis tracker for multi-replica shared state; policies scope to workspace or endpoint; one alert per window to prevent webhook storms | [MLflow blog](https://mlflow.org/blog/gateway-budget-alerts-limits) |
...
| Gateway-level budget enforcement | **Medium-High** | LiteLLM (tag budgets, provider routing), Bifrost (hard limits, 11µs overhead), Portkey (virtual key budgets), Traefik Triple Gate (proactive token estimation, MCP TBAC), MLflow AI Gateway 3.10 (OSS USD budget policies with alert/reject modes, Mar 2026), Databricks Unity AI Gateway (dollar logging + MCP governance), agentgateway (OSS token-bucket with LLM-token counting). Production-ready; rapid innovation continued through Q2 2026. **Caveat (April 2026):** malicious LLM routers in the supply chain can compromise the enforcement point itself — require gateway binary integrity checks and pinned provider endpoints. |
