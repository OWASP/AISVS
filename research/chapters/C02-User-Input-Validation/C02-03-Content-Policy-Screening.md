...
|---|-------------|:-----:|-----------------|----------------------|--------------|
| **2.3.1** | **Verify that** every inbound prompt is scored by a content classifier for violence, self-harm, hate, sexual content, and illegal requests against configurable thresholds, and that prompts exceeding those thresholds are rejected or sanitized before reaching model context. | 1 | Generation of harmful content; policy-violating outputs; liability exposure from producing illegal or dangerous content; reputational damage from harmful AI responses. Relevant MITRE ATLAS techniques: AML.T0054 (LLM Jailbreak), AML.T0051 (Prompt Injection), AML.T0043 (Craft Adversarial Data). | 1. Confirm a content classification service is integrated (e.g., OpenAI Moderation API, Azure AI Content Safety, AWS Bedrock Guardrails, Anthropic content filtering, LLM Guard, Perspective API, IBM Granite Guardian). 2. Verify it runs on both inputs (pre-model) and outputs (post-model). 3. Test with known harmful content across all categories (violence, self-harm, hate, sexual, illegal). 4. Confirm thresholds are configurable per category (not hard-coded). 5. Check that threshold configuration is documented and reviewed periodically. 6. Verify classifiers return numerical confidence scores, not just pass/fail, to support threshold tuning. 7. Verify that prompts exceeding thresholds are rejected (not passed to the model with a warning) or sanitized before reaching model context. | Most major providers offer content classification APIs. OpenAI's `omni-moderation-latest` (GPT-4o-based) is free and covers 11 subcategories across 40+ languages with 42% improvement on multilingual evaluation. Azure AI Content Safety provides 4-level severity scoring (Safe/Low/Medium/High) plus Prompt Shields for jailbreak detection. AWS Bedrock Guardrails offers configurable filter strength across 6 categories including prompt attack detection. For self-hosted models, LLM Guard (MIT license, 2.5M+ downloads) provides 15 input scanners and 20 output scanners. Classifier evasion through paraphrasing, coded language, and encoding tricks remains a known limitation — adversarial paraphrasing achieves 87.88% true positive reduction in some evaluations (2025 research). The Palo Alto Unit 42 study (March 2025) found output-only guardrails blocked just 0–1.6% of harmful responses across major platforms, underscoring the need for input-side screening. As of May 2026, an empirical analysis of evasion attacks across six popular guardrail systems (Azure Prompt Shield, Meta Prompt Guard, ProtectAI, NeMo Guard, Vijil) found that simple character transformations and algorithmic AML evasion can reach up to 100% bypass rates — most guardrails are lightweight NLP classifiers fine-tuned on curated datasets and inherently misaligned with the broader corpora their protected LLMs were trained on. For multimodal systems, the NVIDIA AI Red Team (July 2025) demonstrated semantic prompt injection via visual rebus sequences — emoji-like icon chains (e.g., printer + waving person + globe = "print Hello World") that bypass text-based content filters entirely, exploiting early fusion architectures where vision and text tokens share latent space. The open-source guardrail model landscape is expanding: Llama Guard 3 Vision (Meta, 11B parameters) is the first multimodal safety classifier, covering 13 MLCommons hazard categories for text+image inputs; ExpGuard (ICLR 2026) targets domain-specific content moderation in financial, medical, and legal contexts, outperforming WildGuard by up to 15.3% in response classification; and Galileo Luna-2 achieves 0.95 accuracy with low-latency, low-cost production guardrail metrics. |
| **2.3.2** | **Verify that** prompt content classification is evaluated for unsupported-language abuse and that identified gaps are mitigated through compensating controls such as language detection with rejection, conservative thresholds, or human review routing. | 1 | Content classifiers with limited language coverage create exploitable bypass surfaces — attackers submitting policy-violating content in unsupported languages evade screening entirely. Low-resource languages exhibit ~3x the harmful content generation rate compared to English (ICLR 2024). Code-switching attacks (mixing English with another language mid-sentence) achieve up to 99% bypass rates against English-primary classifiers (AbuseEval-L, 2025; MultiMo-SafetyBench). MITRE ATLAS AML.T0054 (LLM Jailbreak) via language evasion. | 1. Obtain the classifier's documented language coverage list and identify unsupported or low-confidence languages relevant to expected user populations. 2. Test with harmful content in unsupported or low-resource languages (e.g., Swahili, Amharic, Tagalog, Khmer) and verify whether screening fires or passes. 3. Test code-switching inputs that mix English with a target language mid-sentence for known harmful request patterns. 4. Verify compensating controls are in place for identified gaps: language-ID gating (langdetect, fastText, langid.py) with rejection for unsupported languages, conservative threshold lowering for uncertain-language inputs, or routing to human review. 5. Confirm the language coverage assessment and compensating control selection are documented and reviewed when the classifier is updated. | OpenAI's `omni-moderation-latest` documents 40+ language support with a 42% multilingual improvement, but coverage confidence degrades significantly for low-resource languages outside its training distribution. Azure AI Content Safety is trained on 8 languages with broader but undocumented confidence levels for others. Most open-source classifiers (LLM Guard, NeMo Guardrails) are primarily English. Language-ID tools (fastText lid.176, langdetect, langid.py) operate at sub-millisecond latency and can be placed as a pre-classifier gate to detect unsupported languages before scoring. Conservative threshold approach: reduce the reject threshold for inputs where the classifier returns low-confidence scores or where language-ID confidence is below a defined floor. Human review routing is appropriate for borderline cases in regulated deployments where false negatives carry high liability. The EU AI Act's Annex III high-risk AI system obligations (enforceable August 2026) and the CHATBOT Act proposals (April 2026) both implicitly require that content screening is effective across the language populations served, not merely in the classifier's primary training language. |
...
| **Lakera Guard** | Lakera | Prompt injection, jailbreak, PII, data leakage, malicious links | Multilingual | Real-time API security firewall for inputs and outputs; SaaS and self-hosted | Pay-per-call |
...
|------|---------|-------|-------------|
| **Promptfoo** (OpenAI announced acquisition March 2026) | MIT | 50+ vulnerability types; prompt injection, jailbreak, PII leak, toxic content scanning | CI/CD integration via YAML configs and GitHub Actions; guardrails assertions for AWS Bedrock and Azure OpenAI; red-team replay against historical datasets |
...
|-------------|----------|-------|
| **2.3.1** Content classifiers with threshold-based enforcement | **High** | Multiple production-ready commercial APIs (OpenAI free, Azure, AWS, Google, Galileo, Lakera) and open-source options (LLM Guard, Granite Guardian, Llama Guard 3 Vision, ExpGuard). Multimodal safety classification is now available via Llama Guard 3 Vision and ShieldGemma 2. Main gaps: classifier evasion resistance and visual semantic attacks (rebus/emoji-based) that bypass text-only classifiers entirely. |
| **2.3.2** Multilingual classifier gap evaluation | **Low** | Most commercial classifiers document partial multilingual coverage, but formal coverage assessments are rare in practice. Language-ID tooling (fastText, langdetect) is mature and low-latency, making language-detection-with-rejection a straightforward compensating control. Code-switching attack datasets (AbuseEval-L, MultiMo-SafetyBench) provide test vectors but are not yet integrated into standard guardrail evaluation suites. |
| **2.3.3** Pre-model rejection | **Medium** | Architectural pattern is well-understood (gateway not advisory), but multi-turn attacks (crescendo, Bad Likert Judge), token flip attacks (EchoGram), and visual rebus attacks (NVIDIA, July 2025) expose fundamental weaknesses across both text and multimodal modalities. NeMo Guardrails offers unique multi-turn awareness. For agentic systems, output-level controls and action screening are now considered essential alongside input screening. |
...
| [C02-02 Pre-Tokenization Input Normalization](C02-02-Pre-Tokenization-Input-Normalization) | Encoding and obfuscation attacks (Unicode, Base64, homoglyphs) affect both normalization and content classifiers |
| [C05 Access Control & Identity](C05-Access-Control) | ABAC policy resolution depends on identity infrastructure for user attributes (age, region, role) |
| [C09 Orchestration & Agentic Action](C09-Orchestration-and-Agents) | Rejected content must not propagate through agent chains (2.3.3) |
| [C10 MCP Security](C10-MCP-Security) | Content screening must cover MCP tool inputs/outputs; policy-violating content must not propagate to MCP servers (2.3.3) |
| [C13 Monitoring & Logging](C13-Monitoring-and-Logging) | Screening logs (2.3.4) feed into the broader monitoring and SIEM infrastructure |
...
- [C02-01 Prompt Injection Defense](C02-01-Prompt-Injection-Defense) — Prompt injection detection and content policy screening often share guardrail models, evasion tests, and pre-model gateway placement.
- [C11-08 Agent Security Self-Assessment](../C11-Adversarial-Robustness/C11-08-Agent-Security-Self-Assessment) — Agent self-review controls complement action screening when policy-violating inputs could trigger tools or MCP calls.
- [C05-02 Authorization Policy](../C05-Access-Control/C05-02-Authorization-Policy) — Age, region, tenant, and role attributes used by content policy gates depend on reliable authorization policy resolution.
- [C01-05 Data Lineage Traceability](../C01-Training-Data/C01-05-Data-Lineage-Traceability) — Replayable screening logs and classifier tuning depend on trustworthy lineage for test payloads, moderation datasets, and policy changes.
...
| **CHATBOT Act / GUARD Act (proposed U.S.)** | Senate Commerce introduction (Apr 28, 2026); Senate Judiciary approval (Apr 30, 2026) | Proposed requirements include family accounts, parental controls, age verification, non-human-status disclosures, restrictions on AI companions for minors, and penalties for sexual, self-harm, or violent chatbot content involving minors | Proposed federal legislation; not enacted as of May 2, 2026 |
