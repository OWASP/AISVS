...
> **2025-2026 Highlights:** Alignment evaluation now includes assessment for evaluation awareness (C11.1.5), where models may behave differently under testing versus deployment. Agent-specific sections (C11.8, C11.9) address self-modification and autonomous update security as agentic systems become mainstream. As of March 2026, proactive deceptive defenses (ProAct, HoneyTrap) show promising results against multi-turn jailbreaks, while new attack classes — semantic chaining, reasoning-oriented programming (VROP), and production-scale indirect prompt injection — demonstrate that safety filter architectures must evolve beyond per-turn content evaluation.
