...
- Flag lifecycle-script execution (`preinstall`, `postinstall`, `prepare`, import-time package hooks) in AI build and inference dependencies for sandboxed review before promotion.
- In incident drills, include credential rotation for cloud, registry, GitHub, SSH, Kubernetes, and Vault secrets reachable from developer workstations or CI runners that installed affected packages.
...
- [ ] **How should trusted publishing be verified when the workflow is compromised?** — TanStack's May 2026 incident showed that valid OIDC trusted-publisher provenance can still authenticate malicious packages if attacker-controlled code reaches the release runner first. Practical defenses need release-workflow allowlists, cache-boundary isolation, publish-step attestation, and alerts when package metadata is signed by the right identity but from the wrong code path.
