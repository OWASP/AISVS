...
| Mar 2026 | Nginx UI MCP endpoint auth bypass | CVE-2026-33032 (CVSS 9.8) | `/mcp_message` endpoint only applies IP whitelisting with default empty whitelist (treated as "allow all"); unauthenticated attackers can invoke all MCP tools to restart Nginx, create/modify/delete configs. Public PoC available; no patch as of April 2026 |
...
| Argument-level authorization | **Low–Medium** | Traefik TBAC (EA March 2026), Gravitee MCP Proxy (OpenFGA/AuthZen), and Oso (Polar policy-as-code) now offer relationship-based and parameter-level authorization. Oso supports RBAC, ReBAC, and ABAC simultaneously for MCP tool authorization. |
...
| MCP gateway enforcement | **Medium–High** | Kong, Traefik, Azure APIM, IBM ContextForge, AWS AgentCore Gateway, Okta Agent Gateway, Gravitee MCP Proxy, MCP Gateway Registry, and TrueFoundry now provide centralized auth. Gravitee adds OpenFGA/AuthZen for relationship-based authorization. Okta's virtual MCP server aggregates tools with IdP-backed authorization. Adoption accelerating rapidly — gateways are becoming the default enforcement path. |
...
| Proof-of-possession (DPoP/mTLS) | **Very Low** | SEP-1932 (DPoP for MCP) is under active refinement — simplified to use DPoP out-of-the-box with AEAD-encrypted nonces for stateless replay protection — but no MCP implementation ships DPoP yet. mTLS is available at transport layer but breaks through intermediaries. AIP paper proposes Ed25519-based IBCTs with 100% adversarial rejection but is pre-implementation. |
...
- **[C04-04 Secrets & Key Management](C04-04-Secrets-Key-Management.md)** — Credential storage, rotation, and HSM-backed key management directly relevant to MCP token and API key lifecycle
...
- **[C05-06 Autonomous Agent Authorization](C05-06-Autonomous-Agent-Authorization.md)** — Scoped capability tokens and per-action policy evaluation for agents — the authorization layer that complements MCP's OAuth transport
