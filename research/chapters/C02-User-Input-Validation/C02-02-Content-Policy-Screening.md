...
| **2.2.3** | **Verify that** non-text inputs (image/video/audio) are checked for adversarial perturbations, steganographic payloads, hidden or embedded content, or known attack patterns. | 2 | Text-in-image typographic injection (FigStep, AAAI 2025), image-scaling attacks where the payload only emerges after downscaling (Trail of Bits Anamorpher, Aug 2025 — drove Gemini CLI to exfiltrate calendar data), "unseeable" low-contrast screenshot injection (Brave, Oct 2025), steganographic payloads (Odysseus dual-steganography against commercial multimodal systems, arXiv 2512.20168), near-ultrasonic and adversarial audio (Sirens' Whisper, AudioJailbreak), and video-driven prompting (CVPR 2026). The preprocessing pipeline itself becomes the attack surface. | OCR-then-classify every inbound image through the same text guardrails, wrapped in delimiters and marked untrusted. Decode-and-re-encode to destroy pixel-domain stego and strip metadata channels (ExifTool `-all=`, libvips/ImageMagick `-strip`, Pillow re-save) and re-run the production downscaler on a separate path to defuse scaling attacks. Add CDR (OPSWAT MetaDefender Deep CDR) for file-borne hidden content. Use multimodal classifiers where available (Llama Guard 4, ShieldGemma 2, Azure `imageWithText`, Bedrock multimodal toxicity GA). Transcribe-then-classify audio (Whisper, Azure Speech). Red-team with garak `visual_jailbreak` (FigStep), PyRIT image/audio converters, and Promptfoo image/audio/video strategies. | **Medium** for classification, **Low** for steganalysis. No cloud vendor ships real-time steganalysis; open tools (Aletheia, StegExpose, zsteg) are research-grade (~50% accuracy vs. modern embeddings); ART input transforms are defeatable by adaptive attacks. Transcribe-then-classify is blind to the acoustic channel where near-ultrasonic payloads live — audio-native research guardrails (AudioGuard, arXiv 2604.08867, which reads unsafe sound events and speaker identity straight from the waveform) and explainable image guardrails (SafeVision) are starting to close that gap but remain research-grade; Google Model Armor remains text-only. Destruction-by-transformation (re-encode/resize/strip, CDR) is the deployable control — treat detection as best-effort. |
