...
|---|-------------|:-----:|-----------------|----------------------|--------------|
| **11.8.1** | **Verify that** agentic systems include a mechanism to review planned high-risk actions against security policy before execution (e.g., a secondary model, rule-based checker, or structured self-review step). | 2 | Agents executing harmful actions due to adversarial manipulation, hallucinated plans, or goal misalignment. Without pre-execution review, a compromised or confused agent can take irreversible actions (deleting data, sending unauthorized communications, modifying configurations) before anyone notices. MITRE ATLAS AML.T0101 (Data Destruction via AI Agent Tool Invocation) and the OpenClaw inbox-deletion case in the OWASP GenAI Q1 2026 round-up are concrete examples of what this review must catch. | Review agent architecture for pre-execution review mechanisms. Identify which actions are classified as "high-risk" and verify the classification is comprehensive. Test with AgentHarm (110 malicious tasks) and the Agent Red Teaming (ART) benchmark (~4,700 distilled adversarial prompts) to produce repeatable coverage metrics. Exercise the review mechanism with planned actions that violate security policy and confirm they are caught. Verify the review cannot be skipped or bypassed by the agent itself. Test edge cases where individually benign actions compose into harmful sequences. Concrete implementations to inspect include AgentSpec rule enforcement, Agent Behavioral Contracts preconditions, and Microsoft's Agent Governance Toolkit `Agent OS` policy engine (<0.1ms p99). | Defining "high-risk actions" is context-dependent and requires careful analysis of the agent's capabilities. Self-review (asking the same model to evaluate its own plan) is the weakest form of review due to sycophancy and shared failure modes. Secondary models provide better independence but add latency and cost. Rule-based checkers are reliable but brittle and can miss novel attack patterns. A layered approach (rules + model review) is recommended. ART findings show robustness has limited correlation with LLM size, so vendor defaults and bigger-model-as-defense are not sufficient baselines. |
| **11.8.2** | **Verify that** security review mechanisms are protected against manipulation by adversarial inputs (e.g., the review step cannot be overridden or bypassed through prompt injection). | 2 | Adversarial bypass of the security review layer itself. If an attacker can inject instructions that cause the reviewer to approve harmful actions, the review mechanism provides no protection. This is a meta-attack: attacking the defense rather than the primary model. Related to OWASP LLM01 (Prompt Injection), MITRE ATLAS AML.T0054 (LLM Prompt Injection) and AML.T0099 (AI Agent Tool Data Poisoning). Zero-click vectors such as Perplexity Comet PleaseFix and the April 2026 malicious LLM router family show the reviewer's trust boundary must include transport and routed traffic, not just the proposed action. | Attempt to bypass the review mechanism through: injected instructions in tool outputs, manipulated context that redefines security policy, prompt injection targeting the reviewer specifically, social engineering of the review step (e.g., "this action has already been approved"), and zero-click triggers in calendar invites, email, or documents the agent ingests. Verify architectural separation between the review mechanism and untrusted inputs -- the reviewer should evaluate a structured action proposal (tool name, parameters, justification), not raw context. Run NIST CAISI universal-attack corpora and ART transfer attacks to confirm the reviewer does not regress on cross-model adversarial prompts. Confirm the reviewer's policy is not modifiable by the agent or its inputs. | One of the hardest requirements to verify because the attack surface is the same as prompt injection generally. Architectural separation (hardened reviewer with its own system prompt that cannot see untrusted content directly) is the strongest approach. No current approach provides guaranteed protection against all prompt injection variants. NIST CAISI demonstrated at least one successful attack against every frontier model tested and universal attack families transferring across agent implementations -- a "clean" local benchmark does not imply security against targeted attacks. |
| **11.8.3** | **Verify that** security review warnings trigger enhanced monitoring or human intervention workflows for the affected session or task. | 3 | Security warnings that are generated but not acted upon, or that result in a simple block without investigation. A security review warning may indicate an active attack, a system misconfiguration, or a novel failure mode -- all of which require follow-up beyond just blocking the immediate action. The Anthropic Claude espionage (November 2025) and McKinsey Lilli (2026) incidents both showed that isolated blocked actions without session-level escalation allow attackers to continue probing until they find an un-reviewed path. | Review escalation workflows triggered by security review warnings. Verify warnings result in: enhanced logging for the session, notification to security operations (Fortinet FortiSOC, Stellar Cyber, Swimlane integrations), human review queue entry for the task, and session-level risk elevation (tighter controls for subsequent actions in the same session). Verify that agent identity is stable enough across the session to correlate warnings (Ping Identity's March 2026 Runtime Identity Standard provides one concrete approach). Test end-to-end: trigger a security warning and verify escalation workflow execution including SOC notification, logging uplift, and session risk elevation. | Human intervention workflows must be designed for the expected volume of warnings -- too many warnings cause alert fatigue. Session-level risk elevation (treating the entire session as potentially adversarial after one warning) is a useful pattern but may impact legitimate users who trigger false positives. Consider tiered escalation based on warning severity. Few organizations currently publish their tiered-escalation playbooks, so industry benchmarks for alert-to-action latency are still limited. |
...
Verifiers for 11.8.1--11.8.3 can use these benchmarks as continuous-integration gates: a release should not regress on HarmScore, RefusalRate, or ART Attack Success Rate between versions. Critically, reported benchmark performance must be contextualized by the benchmark's own known weaknesses -- the ART finding that attacks transfer across model families means a local "pass" does not guarantee protection against targeted attacks an organization will actually face.
...
- **Ping Identity Runtime Identity Standard** (March 24, 2026): Defines a runtime identity standard for autonomous AI agents, providing cryptographic workload identity binding that the pre-execution review mechanism can check against before authorizing sensitive tool calls. Relevant for 11.8.3 escalation because session-level risk elevation requires a stable agent identity to correlate warnings across actions.

None of these tools alone satisfies the full requirements of 11.8.1 through 11.8.3 -- they are building blocks for a layered architecture that must also include escalation workflows (11.8.3) and hardening against reviewer bypass (11.8.2). The emergence of the Microsoft Agent Governance Toolkit in April 2026 is notable because it is the first major-vendor open-source release that ships coverage for the full OWASP Agentic Top 10 in a single integrated package, though organizations should still validate its policy rules against their own threat model rather than treating vendor defaults as sufficient.
...
- [C13-02 Abuse Detection & Alerting](C13-02-Abuse-Detection-Alerting) -- Downstream alerting pipeline that agent security review warnings should feed into, with behavioral and session-level detection patterns shared across the two controls.
- [C14-01 Kill-Switch & Override Mechanisms](C14-01-Kill-Switch-Override-Mechanisms) -- Out-of-band shutdown and rollback used when pre-execution review identifies active compromise or the escalation path in 11.8.3 determines the session must be halted.
