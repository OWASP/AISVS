...
| **11.4.2** | **Verify that** inputs flagged as anomalous trigger gating actions. | 2 | Detection without enforcement -- a flagged-but-still-processed poisoned input still corrupts output or fires a tool action (ATPA post-tool-call manipulation, MCP rug-pulls, SSH-key exfiltration). Auto-approval amplifies impact: ~84.2% tool-poisoning ASR with auto-approve enabled. | Confirm flagged inputs are blocked, quarantined, down-ranked, or routed to human review -- not merely logged. Exercise gating layers: NeMo Guardrails input/retrieval/execution rails, Lakera Guard L1--L4, Meta LlamaFirewall, Bedrock contextual grounding, trust-weighted retrieval (ASI06), hard-block on tool-hash mismatch, and the auto-approval kill-switch. Inject known-bad content end-to-end and verify the gating action actually fires per route. | Thresholds trade false positives against false negatives (Lakera L1 vs L4); counterfactual gating (RAGuard leave-one-out) costs N+1 inference passes; response-side gating for ATPA is immature; over-aggressive gating erodes clean-task utility (benchmarks show ~94.3% retention as a practical upper bound when stacking rails). |
| **11.4.3** | **Verify that** the safety violation feedback pipeline includes poisoning detection, and human review gates to prevent adversarial manipulation of the improvement mechanism. | 3 | Poisoning of the improvement/feedback loop itself: VIA synthetic-data propagation (~70% inheritance into downstream fine-tunes), self-reinforcing memory loops, adversarial manipulation of RLHF/safety-feedback signals, and Basilisk Venom-style backdoors that survive fine-tuning preprocessing. | Confirm safety-violation and feedback data is screened for poisoning *before* it feeds retraining, fine-tuning, or memory updates, and that human review gates sit on flagged feedback. Check write-time provenance and authorization on memory writes (OWASP Agent Memory Guard, ASI06 provenance/expiry), cross-model verification on high-impact updates (Microsoft CMVK), and traceback for post-incident cleanup (RAGForensics). Audit that unverified entries expire and that forget/rollback is deterministic. | Execute- and Share-phase contamination is poorly tooled (Mnemonic Sovereignty survey); synthetic-data pipeline auditing is immature; human-review gating does not scale to millions of per-user memories without destroying personalization; small published suites (Agent Memory Guard, 55 cases) are a starting point, not production evidence. |
