...
| **Mar 2026** | NVIDIA AI Red Team disclosed semantic prompt injection via visual rebus sequences | Emoji-like icon chains (printer + person + globe) execute code, delete files in agentic systems without any explicit text | Multimodal content classifiers that rely on text-based detection are completely ineffective against visual semantic attacks; early fusion architectures (e.g., Llama 4) are especially vulnerable |
...
| **Mar 2026** | US Congress advanced KOSA and App Store Accountability Act in same week | KOSA requires risk assessments and restricted defaults for users under 17; App Store Act mandates age verification at account level (signed into law in 4 states) | AI content screening systems must support age-gated content delivery and parental oversight tooling |
...
|-------------|----------|-------|
| **2.5.1** Content classifiers | **High** | Multiple production-ready commercial APIs (OpenAI free, Azure, AWS, Google, Galileo, Lakera) and open-source options (LLM Guard, Granite Guardian, Llama Guard 3 Vision, ExpGuard). Multimodal safety classification is now available via Llama Guard 3 Vision and ShieldGemma 2. Main gaps: multilingual coverage in low-resource languages, classifier evasion resistance, and visual semantic attacks (rebus/emoji-based) that bypass text-only classifiers entirely. |
| **2.5.2** Pre-model rejection | **Medium** | Architectural pattern is well-understood (gateway not advisory), but multi-turn attacks (crescendo, Bad Likert Judge), token flip attacks (EchoGram), and visual rebus attacks (NVIDIA, March 2026) expose fundamental weaknesses across both text and multimodal modalities. NeMo Guardrails offers unique multi-turn awareness. For agentic systems, output-level controls are now considered essential alongside input screening. |
| **2.5.3** User-specific policies (ABAC) | **Medium-Low** | Policy engines (OPA, Cedar) are mature, but integration with content screening systems is typically custom work. Galileo Agent Control (Apache 2.0) and Forrester's "agent control plane" market category (December 2025) signal growing interest in centralized policy enforcement for multi-agent deployments. No turnkey solution yet combines content classification with ABAC-based policy resolution. The regulatory surface is expanding rapidly: KOSA and COPPA 2.0 may layer additional age-gated content obligations on AI systems, while the FTC's February 2026 policy statement offers some enforcement flexibility for age verification. |
...
| **KOSA (Kids Online Safety Act)** | Risk assessments, restricted defaults, parental oversight tools | AI systems must restrict default settings for users under 17; mandatory risk assessments for content recommendation algorithms; parental oversight tooling | Advancing through Congress as of March 2026 |
...
- [NVIDIA — Securing Agentic AI: How Semantic Prompt Injections Bypass AI Guardrails (March 2026)](https://developer.nvidia.com/blog/securing-agentic-ai-how-semantic-prompt-injections-bypass-ai-guardrails/)
...
- [FTC COPPA Policy Statement on Age Verification (February 2026)](https://iapp.org/news/a/ftc-policy-statement-signals-coppa-enforcement-flexibility-around-age-verification-practices)
- [KOSA — Kids Online Safety Act](https://fortune.com/2026/03/18/kosa-kids-act-app-store-accountability-act-minors-age-verification/)
