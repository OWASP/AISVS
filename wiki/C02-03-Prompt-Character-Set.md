...
### TokenBreak: Character-Prefix Tokenization Manipulation

TokenBreak (arXiv:2506.07948, June 2025) extends adversarial tokenization into a practical attack against content moderation classifiers. By prepending a single character to words in a prompt, attackers alter how BPE and WordPiece tokenizers segment the input — the classifier sees different tokens and misclassifies malicious content as benign, while the downstream LLM still understands the semantic meaning. WordPiece tokenizers are most vulnerable (mean 55.62% attack success rate), BPE is moderately vulnerable, and Unigram tokenizers are completely resistant (0% success across all tests) because Unigram calculates optimal word splits probabilistically rather than sequentially left-to-right.

**Defense:** Insert a Unigram-based tokenizer as a normalization proxy between input and classification model — this reduced attack success from 33.09% to 12.63% across BPE/WordPiece models without retraining. Character set restrictions complement this defense by limiting the characters available for prefix manipulation, further narrowing the tokenizer's segmentation space.

### Rules File Backdoor (AI Coding Assistants)

Pillar Security disclosed (February 2025) that attackers can inject malicious instructions into AI coding assistant configuration files (`.cursorrules`, Copilot instructions) using invisible Unicode characters — zero-width joiners, bidirectional text markers, and PUA characters. The hidden directives cause the AI agent to generate vulnerable code or insert backdoors, while human reviewers see only innocent configuration text during code review and PR approval. Both Cursor and GitHub declined to treat this as a vendor vulnerability. GitHub implemented a warning banner for hidden Unicode text in files on github.com as of May 2025, though the warning is dismissable and does not block merges. This attack vector is particularly dangerous because shared rule files from community repositories (e.g., cursor.directory) can propagate compromised configurations across thousands of projects.
...
The campaign escalated significantly in early 2026: Socket identified at least 72 malicious Open VSX extensions (mimicking popular linters, formatters, and AI coding tools) that used invisible Unicode to hide payloads, with the threat actor abusing `extensionPack` and `extensionDependencies` to turn benign-looking extensions into transitive delivery vehicles. Compromised extensions steal secrets, credentials, and environment variables, drain cryptocurrency wallets, and abuse infected systems as proxies. The campaign uses Solana transactions as dead-drop resolvers for C2 resilience.

**Detection:** Apply the same character allow-list filtering to code inputs ingested by AI assistants. Scan for Variation Selectors, PUA ranges, and Tags block characters in source code files. Tools like Aikido Safe Chain and Snyk can detect invisible Unicode payloads in package dependencies. GitHub's hidden Unicode text warning (May 2025) provides a platform-level signal during code review, though it is advisory only. ESLint rules and Rust's compiler confusable-identifier warnings offer language-specific detection. The Python `hidden-characters-detector` tool can scan files for invisible Unicode payloads in CI/CD pipelines.
...
| February 2025 | Pillar Security discloses "Rules File Backdoor" — invisible Unicode characters in `.cursorrules` and Copilot instruction files cause AI coding agents to silently generate vulnerable code. Cursor declines to patch (March 6); GitHub adds hidden Unicode warning banner (May 1, 2025) | Demonstrates that character-level attacks extend to AI development tooling configuration, not just runtime prompts |
| June 2025 | TokenBreak research (arXiv:2506.07948) shows that prepending single characters to words bypasses BPE/WordPiece content classifiers (up to 78.93% success on spam detection) while Unigram tokenizers are immune | Motivates Unigram-based tokenizer proxies as a defense layer and highlights that character set restrictions limit prefix manipulation options |
| January–March 2026 | GlassWorm campaign expands to 72+ malicious Open VSX extensions using invisible Unicode, abusing `extensionPack` dependencies as transitive delivery vehicles. Socket and Aikido track ongoing escalation | Invisible Unicode weaponization now spans npm, GitHub repos, and IDE extension marketplaces — defense must cover all code ingestion paths |
| 2025–2026 | Palo Alto Unit 42 documents real-world indirect prompt injection against AI agents browsing the web — 75.8% of detected malicious pages used single injected prompts, with zero-width Unicode, homoglyphs, and encoding schemes used for evasion | Confirms character-level attacks are actively used in wild against production AI agent systems, not just in research |
...
| [hidden-characters-detector](https://github.com/juriku/hidden-characters-detector) | Python | Detects and cleans hidden Unicode characters used for watermarking and prompt injection in files | Useful for CI/CD pipeline scanning of code and config files |
| [SmoothLLM](https://github.com/arobey1/smooth-llm) | Python | Randomized smoothing defense — perturbs input copies and aggregates predictions to detect adversarial inputs | Reduces jailbreak success to <1% across 7 LLMs; character perturbation rate is configurable |
| GitHub Hidden Unicode Warning | Platform | Displays warning banner when files contain hidden Unicode text during code review on github.com | Advisory only (dismissable); triggered by Pillar Security disclosure. Active since May 2025 |
...
| Platform-level invisible Unicode detection | **Medium** — GitHub warning banner live since May 2025; ESLint and Rust compiler provide language-specific checks | Advisory only — does not block merges. No equivalent in GitLab or Bitbucket as of March 2026 |
| Tokenizer-level defenses (Unigram proxy) | **Low** — TokenBreak research (June 2025) proposes Unigram tokenizer proxy as mitigation | Reduces attack success by ~62% without retraining, but no production-ready library exists; requires custom integration |
| Randomized smoothing (SmoothLLM) | **Medium** — published defense reduces jailbreak success to <1% across multiple LLMs | Adds latency from multiple forward passes; character perturbation can degrade response quality on legitimate inputs. Certified Semantic Smoothing (CSS) evolves this to token-level partitioning |
...
- "TokenBreak: Bypassing Text Classification Models Through Token Manipulation." arXiv:2506.07948 (June 2025)
- [Pillar Security: Rules File Backdoor — How AI Code Editors Can Be Weaponized](https://www.pillar.security/blog/new-vulnerability-in-github-copilot-and-cursor-how-hackers-can-weaponize-code-agents) (February 2025)
- [GitHub Changelog: Warning about hidden Unicode text](https://github.blog/changelog/2025-05-01-github-now-provides-a-warning-about-hidden-unicode-text/) (May 2025)
- [GlassWorm Supply-Chain Attack Abuses 72 Open VSX Extensions](https://thehackernews.com/2026/03/glassworm-supply-chain-attack-abuses-72.html) (March 2026)
- [Palo Alto Unit 42: Fooling AI Agents — Web-Based Indirect Prompt Injection in the Wild](https://unit42.paloaltonetworks.com/ai-agent-prompt-injection/)
- [SmoothLLM: Defending Large Language Models Against Jailbreaking Attacks](https://arxiv.org/abs/2310.03684) — randomized smoothing defense reducing jailbreak success to <1%
- [ICU4X 2.0 Release — Unicode Consortium](https://blog.unicode.org/2025/05/icu4x-20-released.html) (May 2025) — modern Unicode property queries with Rust/WASM/JS bindings
...
- TokenBreak demonstrates that BPE/WordPiece classifiers can be bypassed by single-character prefixes while Unigram tokenizers are immune — should AI security architectures mandate Unigram-based classifiers for content moderation, or is a tokenizer-agnostic defense more practical? The Unigram proxy approach (62% reduction in attack success) is promising but adds inference latency and has no production-ready implementation.
- How should invisible Unicode detection scale across the growing attack surface — npm packages, VSCode extensions, GitHub repos, AI assistant config files, and LLM prompts all now require scanning? Is a unified scanning layer feasible, or will each platform need bespoke detection?
