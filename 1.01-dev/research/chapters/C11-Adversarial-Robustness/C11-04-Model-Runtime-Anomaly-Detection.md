...
|---|-------------|:-----:|-----------------|----------------------|--------------|
| **11.4.1** | **Verify that** inputs from external or untrusted sources pass through anomaly detection before model inference. | 2 | Inference-time data poisoning and indirect prompt injection via RAG corpora, tool/MCP responses, persistent memory, and web-scraped context (PoisonedRAG, 97--99% ASR with only 5 injected docs; MITRE ATLAS AML.T0070 RAG poisoning, AML.T0080 memory poisoning; OWASP LLM01/LLM04/LLM08; Greshake et al. indirect injection). A [July 2026 policy-RAG study](https://arxiv.org/abs/2607.04379) achieved 85% context corruption from one query-agnostic poisoned rule (1.6% of its knowledge base), showing that structured rule stores need document-level screening even when the attacker does not know runtime queries. | Enumerate every external ingestion point (retrieval, tool/MCP responses, memory reads, web scrape) and confirm each routes through a pre-inference detector. Look for retriever-stage filtering (GMTP, RAGMask, ProGRank), hybrid BM25+dense retrieval, HTML/Markdown sanitization plus NFKC normalization (OpenRAG-Soc), post-retrieval isolation (RAGDefender), and schema quarantine + SHA-256 tool pinning for MCP (Invariant `mcp-scan`). Replay a labeled adversarial suite (arXiv 2511.15759, 847 cases; the 272k-attempt IPI competition corpus) and confirm inputs are flagged *before* the model consumes them. Re-run the suite after the production chunker and reranker, varying chunk boundaries and retrieval depth; for corpora with a stable domain taxonomy, compare generic outlier detectors with category-spread features such as CLD-KB. | Stealth-optimized attacks (MIRAGE, Eyes-On-Me, Semantic Chameleon) defeat embedding/perplexity thresholds; detection difficulty varies 13--62x by corpus type; the strongest signal (RevPRAG activations, ~98% TPR) needs model internals unavailable to API-only deployments; metadata/multimodal (MM-MEPA) and KG-RAG channels remain largely undefended. CLD-KB's zero-false-positive, 7 ms result was measured on one small, three-category mission-policy corpus, so it is evidence for taxonomy-aware features rather than a general production performance claim. |
| **11.4.2** | **Verify that** inputs flagged as anomalous trigger gating actions. | 2 | Detection without enforcement -- a flagged-but-still-processed poisoned input still corrupts output or fires a tool action (ATPA post-tool-call manipulation, MCP rug-pulls, SSH-key exfiltration, MCP-ITP implicit redirection into a legitimate high-privilege tool). Auto-approval amplifies impact: ~84.2% tool-poisoning ASR with auto-approve enabled, and [CVE-2026-22708](https://github.com/cursor/cursor/security/advisories/GHSA-82wg-qcm4-fp2w) turned Cursor's allowlisted `git branch` into RCE via env-var poisoning. | Confirm flagged inputs are blocked, quarantined, down-ranked, or routed to human review -- not merely logged. Exercise gating layers: [NeMo Guardrails](https://docs.nvidia.com/nemo/guardrails/latest/configure-guardrails/yaml-schema/guardrails-configuration) input/retrieval/execution rails, [Lakera Guard](https://docs.lakera.ai/docs/defenses) L1--L4, Meta LlamaFirewall, Bedrock contextual grounding, trust-weighted retrieval (ASI06), stateful cross-stage trust scoring (MMA-RAG^T), hard-block on tool-hash mismatch, and the auto-approval kill-switch. Enumerate implicitly trusted operations (shell built-ins, env mutations, config writes) and confirm they are gated too. Inject known-bad content end-to-end and verify the gating action actually fires per route. For action-taking systems, adopt the [NetInjectBench](https://arxiv.org/abs/2607.10490) separation of untrusted artifact text from trusted authorization metadata: trace the detector flag through the final executed action, replay both attack cases and legitimately approved high-impact cases, and report unsafe-action and overblocking rates. | Thresholds trade false positives against false negatives (Lakera L1 vs L4); counterfactual gating (RAGuard leave-one-out) costs N+1 inference passes; response-side gating for ATPA is immature; allowlists silently exempt built-ins/env writes (CVE-2026-22708); over-aggressive gating erodes clean-task utility (benchmarks show ~94.3% retention as a practical upper bound when stacking rails). NetInjectBench's policy-gate result assumes intact authorization metadata and covers synthetic, single-step scenarios with six mock tools and three small local models; it does not establish resilience to compromised policy data or delayed, multi-step tool-output poisoning. |
| **11.4.3** | **Verify that** the safety violation feedback pipeline includes poisoning detection and human review gates to prevent adversarial manipulation of the improvement mechanism. | 3 | Poisoning of the improvement/feedback loop itself: VIA synthetic-data propagation (~70% inheritance into downstream fine-tunes), self-reinforcing memory loops and Zombie-Agent injections that survive sliding-window/RAG memory evolution across sessions, adversarial manipulation of RLHF/safety-feedback signals, and Basilisk Venom-style backdoors that survive fine-tuning preprocessing. | Confirm safety-violation and feedback data is screened for poisoning *before* it feeds retraining, fine-tuning, or memory updates, and that human review gates sit on flagged feedback. Check write-time provenance and authorization on memory writes (OWASP Agent Memory Guard, ASI06 provenance/expiry), cross-model verification on high-impact updates (Microsoft CMVK), and traceback for post-incident cleanup (RAGForensics). Confirm screening sits at the memory-write boundary, not just per-session prompt filtering (which Zombie Agents defeats by design). Audit that unverified entries expire and that forget/rollback is deterministic. | Execute- and Share-phase contamination is poorly tooled (Mnemonic Sovereignty survey); synthetic-data pipeline auditing is immature; persistence strategies are engineered to resist truncation and relevance filtering (Zombie Agents); human-review gating does not scale to millions of per-user memories without destroying personalization; small published suites (Agent Memory Guard, 55 cases) are a starting point, not production evidence. |
...
Research from 2025 on **knowledge poisoning attacks to retrieval-augmented generation** demonstrates that RAG systems are highly vulnerable to targeted poisoning even with minimal injection volume. **PoisonedRAG** (USENIX Security 2025) achieved attack success rates of 97% on NQ, 99% on HotpotQA, and 91% on MS-MARCO by injecting only 5 malicious texts per target question into knowledge bases containing millions of texts. This dramatically low injection threshold means that anomaly detection systems (requirement 11.4.1) must operate at individual-document granularity, not just aggregate statistical analysis.

### Expanded Attack Surface: Agents and Tool Outputs

As of 2025-2026, inference-time poisoning now extends beyond RAG corpora to include **MCP server outputs, tool call results, and synthetic data pipelines**. Poisoning can target the entire LLM inference lifecycle: retrieved documents, API responses, web-scraped content, and inter-agent communications. This expanding attack surface means that requirement 11.4.1's anomaly detection must cover all external data ingestion points, not just retrieval pipelines.

### Attention-Based Detection: Attention Tracker

**Attention Tracker** is an emerging defense that detects poisoned inputs by checking whether a retrieved passage diverts attention away from the system prompt in instruction-following attention heads. This approach leverages the internal mechanism by which indirect prompt injections operate -- they hijack attention from the intended instruction to the injected payload. Attention Tracker represents a model-intrinsic detection method that could complement the statistical outlier detection and consistency scoring described in requirement 11.4.1.
...
- **Hybrid retrieval (BM25 + dense vector search)** reduced gradient-optimized co-retrieval success to 0% at balanced alpha values -- a strong first-line defense (requirement 11.4.1)
...
RAGuard's approach is notable because the inference patch catches sophisticated attacks that evade retrieval-level defenses, acting as a safety net for requirement 11.4.2's gating actions. The tradeoff is computational: leave-one-out testing requires N+1 inference passes for N retrieved documents.
...
This is relevant for requirement 11.4.1's threshold tuning because RevPRAG provides a model-intrinsic signal that can calibrate external detection thresholds. The limitation is that it requires access to model internals (activations), ruling out API-only deployments.
...
Lakera Guard's policy model is explicit about the operational tradeoff: L1 is lenient with very few false positives, while L4 is paranoid with higher false positives and fewer false negatives. For 11.4.2, that means an auditor should not just see "Lakera enabled" or "guardrails enabled"; they should see which threshold level is active per route, why that setting matches the risk tolerance, and how false-positive overrides are reviewed and expired.
...
This finding is particularly concerning for requirement 11.4.1: organizations running multimodal RAG need to extend anomaly detection to metadata channels, not just the visual or textual content itself.
...
This represents a blind spot for requirement 11.4.1's anomaly detection, which typically focuses on retrieved documents and tool outputs. Organizations deploying open-weight models should add inference scaffolding inspection to their detection perimeter.
...
This six-layer approach maps to requirements 11.4.1 (detection at layers 1-2) and 11.4.2 (gating actions at layers 2-4), and enables independent false-positive evaluation of each layer. As of March 2026, the addition of post-retrieval isolation (RAGDefender) and activation monitoring (RevPRAG) closes gaps that existed in the earlier five-layer model.
...
This approach is particularly relevant for requirement 11.4.1 because it operates at inference time with no retraining, complements statistical anomaly detection with a mechanistic signal, and provides a detection layer that is orthogonal to retrieval-level defenses like hybrid search or RAGDefender.
...
This incident underscores that the threat model behind requirement 11.4.1 is not theoretical -- adversaries are actively exploiting inference-time poisoning vectors in production systems.
...
The recommended baseline defenses are operationally concrete and map directly to requirement 11.4.1's "before model inference" timing:
...
This framework is directly relevant to requirements 11.4.1 (schema quarantine and pinning are pre-inference anomaly checks against tool-side poisoning) and 11.4.2 (the gating action for a mismatched hash is, by design, a hard block rather than capability degradation).
...
The paper is notable because it quantifies that basic web-hygiene practices most RAG deployments skip provide approximately a 5x reduction in attack success with negligible cost -- a high-yield, low-effort baseline for requirement 11.4.1.
...
These approaches are notable because they (a) require no fine-tuning of the generator, (b) operate before any LLM inference, and (c) compose with later-stage defenses such as RAGDefender, SDAG, and NPAS/AV. For requirement 11.4.1, GMTP and RAGMask are particularly attractive when the retriever pipeline is the only component under the defender's control -- a common situation for teams that consume managed LLM APIs.
...
The 2026 framework shift -- **from model-centric attacks to execution-layer exposure** -- directly informs stress testing under requirement 11.4.1. Detectors built for RAG corpora alone miss the agent memory, thread, and tool-publishing vectors that dominate recent real-world incidents.
...
The recommendations read like an audit checklist for requirements 11.4.1 and 11.4.2:
...
[**GhostWriter**](https://arxiv.org/abs/2607.06595) targets tool-using personal agents without direct access to either the agent or its memory store. An attacker hides a payload in an untrusted input such as email; the agent saves it as memory during the injection phase, and a later benign task retrieves and activates it. Across the tested agent architectures, the paper reports roughly 98% average injection and 60% average activation. That separation matters for 11.4.1 because the stored item is still untrusted external input when a later task retrieves it for model inference; screening only the original email or tool response leaves the delayed retrieval path exposed.

The proposed AM-Sentry pairs a memory-saving policy with a memory-retrieval screen. A focused 11.4.1 audit should seed hidden payloads through every untrusted tool-input class, then run later benign tasks that retrieve the same topic and confirm the retrieval screen evaluates each memory before it enters model context. For 11.4.2, force the screen to flag a seeded memory and verify that the configured block or quarantine action actually withholds it; logging a flag while continuing inference fails the control. The paper's 16 synthetic attack scenarios are useful seeds, not a production-scale false-positive or coverage benchmark.

### Taxonomy-Aware Detection for Structured Policy RAG (July 2026)

The July 2026 [**CLD-KB** study](https://arxiv.org/abs/2607.04379) tested query-agnostic semantic poisoning against a policy-aware RAG system used for mission orchestration. A single injected rule -- 1.6% of the knowledge base -- corrupted the retrieved context for 17 of 20 evaluated prompts (85%); attack benefit saturated at a 7.7% poisoning rate. The poisoned rule packed vocabulary from three policy categories so it would rank across operator-query types without knowing the query in advance. This is a direct 11.4.1 failure: the adversarial rule crosses the retrieval boundary and reaches the decision model without being recognized as anomalous.

CLD-KB gates retrieved rules before inference using two signals: a One-Class SVM boundary detector and a category-spread detector that flags rules spanning an implausible mix of Workflow, Rules of Engagement, and Capability vocabulary. The paper reports 100% context integrity, no false positives, and 7 ms overhead in its evaluation. Those results are promising for small, structured rule stores, but the category-spread signal depends on a stable domain taxonomy and the evaluation covers one specialized corpus. An audit should therefore reproduce the attack and clean-rule workload on the deployment's own taxonomy rather than transferring the reported thresholds.

### Chunking and Reranking Are Part of the Adversarial Test Boundary (June 2026)

[**When Poison Fails After Retrieval**](https://arxiv.org/abs/2606.11265) shows why a detector benchmark that stops at dense-retrieval rank can misstate production risk. Existing document-level poisons often lose effectiveness after chunking fragments their adversarial signal or after rerankers favor locally coherent, answer-bearing passages. The proposed CRCP attack explicitly optimizes retrieval relevance, reranker consistency, and chunk-boundary robustness, and remains more effective across realistic multi-stage pipelines than attacks optimized only for initial retrieval.

For 11.4.1, this cuts both ways: chunking and reranking may suppress a published attack, but they are not dependable anomaly detectors because adaptive attacks can model them. Verification should preserve stage-by-stage evidence -- pre-chunk document, produced chunks, initial rank, reranker score, detector decision, and final gated context -- and replay poisons across the actual chunk sizes and rerankers used in production. For 11.4.2, the test passes only when the final context is blocked or quarantined; a favorable rank change is not itself a gating action.

### NetInjectBench: Measuring the Detector-to-Action Boundary (July 2026)

[**NetInjectBench**](https://arxiv.org/abs/2607.10490) evaluates whether indirect prompt injections in tickets, alerts, logs, runbooks, knowledge-base snippets, and ChatOps messages can cause unsafe network-operation tool calls. Its 130 scenarios deliberately separate untrusted artifact text, trusted policy metadata, and evaluation-only labels, and include ten legitimately approved high-impact changes so that a defense cannot score well simply by blocking every privileged action. Across 240 attack instances on three local models, naive execution produced an 82.50% unsafe tool-action rate. Prompt-only safety, Self-Reminder, Spotlighting, and a two-pass model judge reduced that rate to 25.63%, 21.67%, 18.33%, and 10.00%, respectively, but each left residual unsafe actions.

The deterministic metadata-aware gate treated the model's tool call as a proposal and checked approval status, maintenance window, tool, device, patch, and change-request identity before execution. It produced no unsafe final actions in the evaluated attacks (95% Wilson upper bound 1.58%) while preserving all approved-change utility; static allowlisting reached a 5.00% unsafe-action rate but blocked every approved change. For 11.4.2, this supplies a useful audit pattern: preserve the anomaly decision, proposed action, gate decision, final action, and block reason in one trace, then measure both unsafe-action and overblocking rates. The result is not evidence that metadata-aware gates detect poisoned content. It assumes the policy channel is intact, uses synthetic single-step scenarios with six mock tools, evaluates three small models at deterministic decoding, and leaves delayed tool-output poisoning and multi-step workflows for future work. The paper also states that its benchmark artifacts will not be public until acceptance, so teams currently need to reproduce the scenario pattern rather than rely on a released suite.
...
**ISO/IEC 27090** ("Cybersecurity -- Artificial Intelligence -- Guidance for addressing security threats and compromises to AI systems") reached FDIS -- the final ballot stage before publication -- in early 2026, with publication expected in the second half of 2026. The draft explicitly covers data poisoning including exploitation of continuous learning during operation, and is unusually candid for a standards document: it states that detecting poisoning is necessary but "often difficult." Once published it becomes the ISO-track reference for poisoning detection controls and a plausible input to EU AI Act conformity assessment. For 11.4.1, the practical takeaway is that honestly documenting detector limitations and measured false-positive rates aligns with where the international standards track is heading, while claiming comprehensive detection coverage does not.

### Zombie Agents: Self-Reinforcing Injection That Survives Memory Evolution (February 2026)

**Zombie Agents** (arXiv 2602.15654, February 17, 2026) is the clearest demonstration to date that memory evolution itself converts a one-time indirect injection into persistent compromise -- the core threat behind requirement 11.4.3's improvement-mechanism integrity. The attack has two phases: an **infection phase** where the agent encounters poisoned web content during a legitimate task and writes the payload to long-term memory through normal update processes, and a **trigger phase** where the payload reactivates across later sessions to drive attacker-chosen tool behavior while preserving normal task performance (so utility metrics stay clean and the compromise evades casual review).

What makes it directly relevant to detection design is that the authors engineer **mechanism-specific persistence strategies** for the two dominant memory implementations -- sliding-window memory and retrieval-augmented memory -- so the payload resists both context-window truncation and relevance filtering. In other words, the naive mitigations (let old context fall out of the window; only retrieve topically relevant memories) are exactly the mechanisms the attack is built to defeat. The paper's own conclusion is that "defenses focused only on per-session prompt filtering are not sufficient for self-evolving agents," which is the empirical justification for 11.4.3's insistence that screening sit at the memory-write and feedback-ingestion boundary, not just on the live prompt. This is the self-reinforcing-loop variant the requirement names, now with a working attack and a persistence model rather than a hypothetical.

### Stateful Trust Inference: MMA-RAG^T (February 2026)

**MMA-RAG^T** ("Adversarial Intent is a Latent Variable: Stateful Trust Inference for Securing Multimodal Agentic RAG," arXiv 2602.21447, February 24, 2026) is a defense built specifically for the failure mode that breaks point-in-time detectors: distributed attacks whose individual steps look benign but whose *sequence* across retrieval, planning, and generation is malicious. It models security as a Partially Observable Markov Decision Process in which adversarial intent is a hidden variable, and runs a model-agnostic **Modular Trust Agent** that maintains belief states across pipeline stages rather than scoring each stage in isolation.

Reported results across 43,774 test instances: a **6.50x average reduction in attack success rate** versus undefended systems, with minimal utility impact. The ablation is the useful part for 11.4.1 audit design -- statefulness alone contributed 26.4 percentage points and spatial (cross-stage) coverage another 13.6 points, which quantifies how much detection power comes specifically from correlating signals *across* the ingestion pipeline instead of treating retrieval, tool output, and generation as independent checkpoints. For deployments that already run per-stage rails (NeMo Guardrails input/retrieval/execution rails), this is evidence that a stateful overlay correlating those rails catches attacks each rail misses individually -- and that 11.4.2 gating decisions improve when they consume an accumulated belief state rather than a single-stage flag.

### MCP-ITP: Implicit Tool Poisoning via High-Privilege Tool Hijacking (January 2026)

**MCP-ITP** (arXiv 2601.07395, January 12, 2026) is the first automated, adaptive framework for *implicit* tool poisoning in MCP, and it sharpens the threat model for 11.4.1's tool-side detection. Unlike explicit tool poisoning (where the malicious tool is the one invoked, the case MCPTox and OWASP MCP03 already cover), implicit poisoning leaves the poisoned tool **uninvoked**: instructions embedded in its metadata steer the agent into calling a *separate, legitimate, high-privilege* tool to carry out the malicious operation. The framework chains three LLMs -- an attacker LLM that iteratively optimizes poisoned tool descriptions, a malicious-tool detector LLM, and an effectiveness evaluator -- to automatically search for descriptions that survive review while redirecting agent behavior.
...
This is the execution-layer analogue of the ATPA/MCP-ITP pattern: the dangerous action is laundered through a trusted, approved channel. Cursor's fix was to require explicit approval for any command the parser cannot fully classify. For requirement 11.4.2, the lesson is that an allowlist is only a valid gating control if the set of *implicitly trusted* operations (shell built-ins, environment mutations, configuration writes) is also enumerated and gated -- otherwise the allowlist becomes the delivery vehicle, the same inversion seen in the earlier CVE-2025-54136 Cursor MCP-config swap and the 84.2% auto-approval tool-poisoning success rate.

---

## Related Standards & References

- [MITRE ATLAS AML.T0020 -- Poison Training Data](https://atlas.mitre.org/) -- Data poisoning attack techniques (applicable to inference-time context)
- [MITRE ATLAS AML.T0018 -- Backdoor ML Model](https://atlas.mitre.org/) -- Backdoor and poisoning attack techniques
...
- [OWASP LLM01:2025 Prompt Injection](https://genai.owasp.org/llmrisk/llm01-prompt-injection/) -- Indirect prompt injection via poisoned context is closely related
...
- [NVIDIA NeMo Guardrails Configuration](https://docs.nvidia.com/nemo/guardrails/latest/configure-guardrails/yaml-schema/guardrails-configuration) -- Input, output, retrieval, and execution rails for measuring guardrail behavior by stage
...
- [When Agents Remember Too Much: Memory Poisoning Attacks on Large Language Model Agents (arXiv 2607.06595, July 2026)](https://arxiv.org/abs/2607.06595) -- GhostWriter's delayed memory activation and AM-Sentry's paired admission-policy and retrieval-screen defense
- [Knowledge Base Poisoning Attacks and Defense for Policy-Aware LLM-RAG Framework (arXiv 2607.04379, July 2026)](https://arxiv.org/abs/2607.04379) -- Query-agnostic policy-rule poisoning and CLD-KB's taxonomy-aware pre-inference detector; one injected rule caused 85% context corruption in the evaluated corpus
- [When Poison Fails After Retrieval: Revisiting Corpus Poisoning under Chunking and Reranking Pipelines (arXiv 2606.11265, June 2026)](https://arxiv.org/abs/2606.11265) -- CRCP models chunk boundaries and reranker consistency, exposing the realism gap in retrieval-only poisoning evaluations
- [NetInjectBench: Benchmarking Indirect Prompt Injection in Tool-Using Large Language Model Agents for Network Operations (arXiv 2607.10490, July 2026)](https://arxiv.org/abs/2607.10490) -- Paired attack and approved-change scenarios for measuring unsafe final actions and overblocking at the execution gate
...
- [Zombie Agents: Persistent Control of Self-Evolving LLM Agents via Self-Reinforcing Injections (arXiv 2602.15654, February 2026)](https://arxiv.org/abs/2602.15654) -- Memory-evolution attack with mechanism-specific persistence against sliding-window and RAG memory; shows per-session prompt filtering is insufficient for self-evolving agents
- [Adversarial Intent is a Latent Variable: Stateful Trust Inference for Securing Multimodal Agentic RAG -- MMA-RAG^T (arXiv 2602.21447, February 2026)](https://arxiv.org/abs/2602.21447) -- POMDP-based cross-stage trust overlay; 6.5x ASR reduction over 43,774 instances, with statefulness contributing 26.4pp and spatial coverage 13.6pp
- [MCP-ITP: An Automated Framework for Implicit Tool Poisoning in MCP (arXiv 2601.07395, January 2026)](https://arxiv.org/abs/2601.07395) -- Implicit tool poisoning where the poisoned tool stays uninvoked and instead redirects the agent into a legitimate high-privilege tool; three-LLM optimization loop
- [The Agent Security Paradox: When Trusted Commands in Cursor Become Attack Vectors (Pillar Security)](https://www.pillar.security/blog/the-agent-security-paradox-when-trusted-commands-in-cursor-become-attack-vectors) -- Analysis of CVE-2026-22708 allowlist bypass via shell built-ins and environment-variable poisoning
- [CVE-2026-22708 -- Cursor Terminal Tool Allowlist Bypass via Environment Variables (GitHub advisory GHSA-82wg-qcm4-fp2w)](https://github.com/cursor/cursor/security/advisories/GHSA-82wg-qcm4-fp2w) -- Auto-Run allowlist bypass enabling indirect-prompt-injection RCE; fixed in Cursor 2.3 by gating unclassifiable commands
- [Towards Secure Retrieval-Augmented Generation: A Comprehensive Review of Threats, Defenses and Benchmarks (arXiv 2603.21654, March 2026)](https://arxiv.org/abs/2603.21654) -- Consolidated 2026 survey of RAG poisoning threats, defenses, and evaluation benchmarks
- [Adversa AI: Top Agentic AI Security Resources, June 2026](https://adversa.ai/blog/top-agentic-ai-security-resources-june-2026/) -- June 2026 roundup of agent RCEs, MCP flaws, memory-poisoning attacks, and emerging agent defenses
- [Help Net Security: Prompt Injection Still Drives Most Agentic AI Security Failures in Production (June 11, 2026)](https://www.helpnetsecurity.com/2026/06/11/owasp-prompt-injection-ai-security-failures/) -- OWASP-aligned analysis on why indirect injection and memory poisoning remain the dominant production failure modes
...
- GhostWriter separates memory admission from later activation. Which memories should be automatically blocked, quarantined for manual release, or allowed with monitoring when reviewing every personalized memory is impractical?
- CLD-KB relies on vocabulary spread across a fixed domain taxonomy. Can taxonomy-aware anomaly features remain useful when policies legitimately span categories or the taxonomy evolves, without converting cross-cutting rules into false positives?
- CRCP models chunking and reranking jointly. Which stage-level traces and test transformations are sufficient to show that a pre-inference detector remains effective when chunk size, overlap, retrieval depth, or reranker model changes?
- NetInjectBench assumes trusted authorization metadata and measures a single tool-selection step. How should the same detector-to-gate trace and overblocking metrics be extended when a safe read returns poisoned content that redirects a later action, or when the policy channel itself is stale or compromised?
...
- Zombie Agents engineers persistence strategies specifically against sliding-window truncation and retrieval relevance filtering -- the two mechanisms a naive memory hygiene policy relies on. If both natural decay paths can be defeated, what affirmative write-time signal (provenance, authorization scope, self-reinforcement-rate limits) is strong enough to deny persistence without a human gate on every memory write?
- MMA-RAG^T shows that cross-stage statefulness contributes more detection power (26.4pp) than spatial coverage (13.6pp), implying that correlating signals over time matters more than adding checkpoints. How should an auditor measure and require statefulness in a deployed detector, given that most production guardrail stacks (NeMo, Lakera, LlamaFirewall) still score each stage independently?
- MCP-ITP's implicit poisoning leaves the malicious tool uninvoked and redirects the agent into a legitimate high-privilege tool, so tool-hash pinning and argument inspection both pass. What cross-tool provenance signal -- tracking which tool description influenced an invocation decision -- can be captured without model-internal instrumentation, and is it feasible for hosted-API agents?
- CVE-2026-22708 shows that allowlists silently exempt implicitly trusted operations (shell built-ins, environment mutations). Is there a principled way to enumerate the full implicitly trusted operation set for an agent runtime, or does every new execution primitive reopen the same gating gap?

---

## Related Pages

- [C02-01 Prompt Injection Defense](../C02-User-Input-Validation/C02-01-Prompt-Injection-Defense.md) -- Direct prompt injection at the user-input boundary is the sibling problem to the indirect, retrieved-context injection screened here; the two detection layers should share signatures.
