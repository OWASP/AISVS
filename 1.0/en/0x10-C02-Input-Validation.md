...
| # | Description | Level |
| :--------: | ------------------------------------------------------------------------------------------------------------------- | :---: |
| **2.3.1** | **Verify that** a content classifier scores every inbound prompt for violence, self-harm, hate, sexual content, and illegal requests against configurable thresholds, and rejects or sanitizes prompts that exceed those thresholds before they reach model context. The classifier is evaluated for robustness against prompts submitted in languages outside the system's intended operational scope, with gaps mitigated by compensating controls such as language detection with rejection, conservative threshold adjustment, or human review routing. | 1 |
