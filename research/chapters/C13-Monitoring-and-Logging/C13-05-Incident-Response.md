...
As of 2025, ATLAS added 14 new techniques specifically for AI agents, covering risks including prompt injection, memory manipulation, and tool abuse. The **January 2026 ATLAS v5.3.0 update** extended this further: five new agent-focused technique IDs that IR playbooks should enumerate directly as expected attack paths:
...
ATLAS v5.3.0 also added three case studies covering MCP server compromise, indirect prompt injection via MCP channels, and malicious AI agent deployment — exercise these scenarios in tabletop drills if the organization operates MCP servers. The Center for Threat-Informed Defense's **OpenClaw investigation (February 2026, report PR-26-00176-1)** mapped real AI-agent incident patterns to ATLAS TTPs and identified the chokepoint techniques adversaries rely on; use it to prioritize detection and response coverage.
...
- **MITRE ATLAS v5.3.0 (January 2026)** -- Added AML.T0096-T0101 agent techniques, SesameOp case study (AML.CS0042), and MCP-focused case studies ([zenity.io](https://zenity.io/blog/current-events/mitre-atlas-ai-security))
...
- [C13-02-Abuse-Detection-Alerting](C13-02-Abuse-Detection-Alerting) — The detection pipeline that feeds this IR process; abuse detection alerts should trigger the AI-specific playbooks described here.
- [C14-01-Kill-Switch-Override-Mechanisms](C14-01-Kill-Switch-Override-Mechanisms) — Out-of-band shutdown and rollback controls that IR teams invoke during high-severity containment actions described in the decision matrix above.
